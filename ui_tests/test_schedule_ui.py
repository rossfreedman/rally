...
from conftest import wait_for_page_load, take_screenshot_on_failure, assert_toast_message
...
class TestSchedulePageUI:
    ...
    def test_schedule_page_loads(self, authenticated_page, flask_server):
        ...
        for url in schedule_urls:
            try:
                ...
                if any(keyword in page.content().lower() for keyword in ['schedule', 'match', 'opponent', 'court']):
        ...
        schedule_containers = [
            'table', '.schedule-table', '.schedule-list', 
            '[data-testid="schedule"]', '#schedule-container'
        ]
    ...
    def test_schedule_displays_match_information(self, authenticated_page, flask_server):
        ...
        assert any(info_found.values()), f"No match information found. Checked: {info_found}"
...
class TestScheduleDisplay:
    ...
    def test_date_formatting(self, authenticated_page, flask_server):
        ...
        date_selectors = [
            '.date', '.match-date', '[data-testid="date"]',
            'td:nth-child(1)', '.schedule-date'  # Common table positions
        ]
    ...
    def test_time_formatting(self, authenticated_page, flask_server):
        ...
        time_selectors = [
            '.time', '.match-time', '[data-testid="time"]',
            'td:nth-child(2)', '.schedule-time'  # Common table positions
        ]
    ...
    def test_opponent_display(self, authenticated_page, flask_server):
        ...
        opponent_selectors = [
            '.opponent', '.team', '.vs', '[data-testid="opponent"]',
            'td:nth-child(3)', '.schedule-opponent'
        ]
        ...
        if opponents_found:
            ...
            for opponent_text in opponents_found:
                assert len(opponent_text) > 2, f"Opponent info should be meaningful: {opponent_text}"
...
class TestScheduleInteraction:
    ...
    def test_schedule_row_clickable(self, authenticated_page, flask_server):
        ...
        clickable_selectors = [
            'tr:not(:first-child)', '.schedule-row', '.match-row',
            'a[href*="match"]', '[data-testid="match-row"]'
        ]
    ...
    def test_schedule_filtering(self, authenticated_page, flask_server):
        ...
        filter_selectors = [
            'select[name*="month"]', 'select[name*="team"]', 'select[name*="filter"]',
            '.filter-dropdown', '[data-testid="filter"]'
        ]
    ...
    def test_schedule_refresh(self, authenticated_page, flask_server):
        ...
        refresh_selectors = [
            ...
            '[data-testid="refresh"]', '.refresh-button'
        ]
...
class TestScheduleResponsive:
    ...
    def test_mobile_schedule_view(self, authenticated_page, flask_server):
        ...
        schedule_visible = (
            ...
            page.locator('.schedule-list').count() > 0 or
            page.locator('[data-testid="schedule"]').count() > 0
        )
        ...
        mobile_optimized = any(
            ...
            for selector in mobile_optimizations
        )
    ...
    def test_tablet_schedule_view(self, authenticated_page, flask_server):
        ...
        schedule_elements = page.locator('table, .schedule-list, [data-testid="schedule"]')
...
class TestScheduleNavigation:
    ...
    def test_navigate_to_schedule_from_mobile(self, authenticated_page, flask_server):
        ...
        schedule_nav_selectors = [
            'a[href*="schedule"]', 'button:has-text("Schedule")',
            '[data-testid="schedule-nav"]', '.nav-schedule'
        ]
    ...
    def test_schedule_breadcrumb_navigation(self, authenticated_page, flask_server):
        ...
        breadcrumb_selectors = [
            '.breadcrumb', '.breadcrumbs', '[data-testid="breadcrumb"]',
...
class TestSchedulePerformance:
    ...
    def test_schedule_loading_time(self, authenticated_page, flask_server):
        ...
        assert load_time < 5.0, f"Schedule should load within 5 seconds, took {load_time:.2f}s"
...
class TestScheduleAccessibility:
    ...
    def test_schedule_table_accessibility(self, authenticated_page, flask_server):
        ...
        if tables.count() > 0:
            ...
            table_labeled = (
                first_table.locator('caption').count() > 0 or
                first_table.get_attribute('aria-label') or
                first_table.get_attribute('aria-labelledby')
            )
            
            print(f"Schedule table accessibility - Headers: {headers.count()}, Labeled: {table_labeled}")
    ...
    def test_keyboard_navigation_schedule(self, authenticated_page, flask_server):
        ...
        print(f"Keyboard navigation - Focused element: {focused_element}, Works: {navigation_works}")
...
class TestScheduleErrorHandling:
    ...
    def test_schedule_no_data_state(self, page, flask_server, ui_test_database):
        ...
        empty_state_selectors = [
            ':has-text("No matches")', ':has-text("No schedule")',
        ...
        has_schedule_data = page.locator('table tr:not(:first-child), .schedule-row').count() > 0
        
        result = empty_state_found or has_schedule_data
        print(f"Schedule state - Empty state: {empty_state_found}, Has data: {has_schedule_data}")
    ...
    def test_schedule_error_handling(self, authenticated_page, flask_server):
        ...
        page.route("**/schedule*", lambda route: route.fulfill(status=500, body="Server Error"))
        
        page.goto(f"{flask_server}/schedule")
        
        # Look for error messaging
        error_selectors = [
            ':has-text("Error")', ':has-text("Unable to load")',
