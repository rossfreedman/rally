...
def compare_databases():
    ...
    if not local_db_url or not railway_db_url:
        print(
            "Error: Both LOCAL_DATABASE_URL and DATABASE_URL environment variables must be set"
        )
        return
    ...
    try:
        ...
        if local_tables != railway_tables:
            ...
            print(
                f"Both databases have the same tables: {', '.join(sorted(local_tables))}"
            )
        ...
        for table in sorted(common_tables):
            ...
            try:
                ...
                with local_conn.cursor(cursor_factory=DictCursor) as local_cursor:
                    ...
                    local_dict = {
                        str(row[primary_key]): dict(row) for row in local_data
                    }
                ...
                with railway_conn.cursor(cursor_factory=DictCursor) as railway_cursor:
                    ...
                    railway_dict = {
                        str(row[primary_key]): dict(row) for row in railway_data
                    }
                ...
                for record_id in all_ids:
                    ...
                    if local_record is None:
                        differences.append(
                            [record_id, "Missing in Local", "Present in Railway"]
                        )
                    elif railway_record is None:
                        differences.append(
                            [record_id, "Present in Local", "Missing in Railway"]
                        )
                    else:
                        # Compare all fields
                        diff_fields = []
                ...
                if differences:
                    ...
                    print(
                        tabulate(
                            differences,
                            headers=["ID", "Local Database", "Railway Database"],
                        )
                    )
                else:
