...
def test_youtube_api():
    ...
    try:
        ...
        for technique_name, technique_data in list(data.items())[
            :5
        ]:  # Test first 5 techniques
            ...
            for video in videos:
                ...
                if not video_id:
                    ...
                    invalid_urls.append(
                        (technique_name, title, url, "Invalid URL format")
                    )
                    continue
                        ...
                        if status["privacyStatus"] == "public":
                            ...
                            invalid_urls.append(
                                (
                                    ...
                                    title,
                                    url,
                                    f"Not public: {status['privacyStatus']}",
                                )
                            )
                    else:
                        print(f"   ❌ Video not found or unavailable")
                        invalid_urls.append(
                            (technique_name, title, url, "Video not found")
                        )
        ...
        print(
            f"Success rate: {(valid_count/test_count)*100:.1f}%"
            if test_count > 0
            else "No videos tested"
        )
