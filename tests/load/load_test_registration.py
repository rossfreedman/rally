...
class RallyUser(HttpUser):
    ...
    def load_test_data(self):
        """Load scraped test data for realistic scenarios"""
        try:
            with open('../fixtures/scraped_players.json', 'r') as f:
    ...
    def view_schedule(self):
        ...
        with self.client.get('/api/schedule',
                           catch_response=True) as response:
            if response.status_code == 200:
                response.success()
                try:
    ...
    def check_availability(self):
        ...
        with self.client.get('/availability',
                           catch_response=True) as response:
            if response.status_code == 200:
                response.success()
    ...
    def create_poll(self):
        ...
        poll_choices = {
            ...
            "Preferred practice frequency?": ["Once a week", "Twice a week", "As needed"]
        }
        ...
        with self.client.post('/api/polls',
                             json=poll_data,
                             catch_response=True) as response:
            if response.status_code == 201:
                response.success()
    ...
    def vote_on_poll(self, poll_id, choices):
        ...
        with self.client.get(f'/api/polls/{poll_id}',
                           catch_response=True) as response:
            if response.status_code == 200:
                try:
                    ...
                    if poll_choices:
                        ...
                        vote_data = {
                            'choice_id': choice['id'],
                            'player_id': random.randint(1, 20)  # Assume players 1-20 exist
                        }
                        
                        with self.client.post(f'/api/polls/{poll_id}/vote',
                                             json=vote_data,
                                             catch_response=True) as vote_response:
                            if vote_response.status_code == 200:
                                vote_response.success()
                            else:
                                vote_response.failure(f"Voting failed: {vote_response.status_code}")
                except:
                    pass
    ...
    def view_team_polls(self):
        ...
        with self.client.get(f'/api/polls/team/{team_id}',
                           catch_response=True) as response:
            if response.status_code == 200:
                response.success()
                try:
    ...
    def search_players(self):
        ...
        with self.client.post('/api/players/search',
                             json=search_data,
                             catch_response=True) as response:
...
class AdminUser(HttpUser):
    ...
    def on_start(self):
        ...
        with self.client.post('/api/login',
                             json=admin_credentials) as response:
            if response.status_code == 200:
    ...
    def view_system_stats(self):
        ...
        with self.client.get('/api/admin/stats',
                           catch_response=True) as response:
        ...
        for i in range(random.randint(1, 3)):
            ...
            with self.client.post('/api/register',
                ...
                if response.status_code in [201, 409]:  # Success or duplicate
                    ...
                    response.failure(f"Burst registration failed: {response.status_code}")
