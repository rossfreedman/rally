...
class TestPollModel:
    ...
    def test_poll_relationships(self, db_session, test_user, test_team):
        ...
        poll = Poll(
            team_id=test_team.id, created_by=test_user.id, question="Test question?"
        )
...
class TestPollAPI:
    ...
    def test_create_poll_api(
        self, authenticated_client, db_session, test_user, test_team
    ):
    ...
    def test_vote_on_poll(
        self, authenticated_client, db_session, test_user, test_player, test_team
    ):
        ...
        vote_data = {"choice_id": choice.id, "player_id": test_player.id}
...
class TestPollVoting:
    ...
    def test_single_vote_per_player(
        self, db_session, test_user, test_player, test_team
    ):
        ...
        # First vote
        vote1 = PollResponse(
            poll_id=poll.id, choice_id=choice1.id, player_id=test_player.id
        )
        ...
        vote2 = PollResponse(
            poll_id=poll.id, choice_id=choice2.id, player_id=test_player.id
        )
    ...
    def test_vote_counting(self, db_session, test_user, test_team, multiple_players):
        ...
        for i, player in enumerate(multiple_players):
            ...
            vote = PollResponse(
                poll_id=poll.id, choice_id=choice.id, player_id=player.id
            )
            db_session.add(vote)
        ...
        saturday_votes = (
            db_session.query(PollResponse)
            .filter(
                PollResponse.poll_id == poll.id, PollResponse.choice_id == choice1.id
            )
            .count()
        )
        ...
        sunday_votes = (
            db_session.query(PollResponse)
            .filter(
                PollResponse.poll_id == poll.id, PollResponse.choice_id == choice2.id
            )
            .count()
        )
...
class TestPollValidation:
    ...
    def test_poll_choice_validation(self, db_session, test_user, test_team):
        ...
        poll = Poll(
            team_id=test_team.id, created_by=test_user.id, question="Valid question?"
        )
        db_session.add(poll)
        db_session.flush()
...
class TestPollSecurity:
    ...
    def test_vote_permission_check(
        self, authenticated_client, db_session, test_user, test_player, test_team
    ):
        ...
        vote_data = {"choice_id": choice.id, "player_id": 99999}  # Non-existent player
    ...
    def test_sql_injection_in_poll(
...
class TestPollLifecycle:
    ...
    def test_full_poll_workflow(
        ...
        vote_data = {"choice_id": choice_id, "player_id": test_player.id}
...
class TestPollPerformance:
    ...
    def test_large_poll_query_performance(
        self, db_session, test_user, test_team, multiple_players
    ):
        ...
        for poll in polls:
            for j in range(3):  # 3 choices per poll
                choice = PollChoice(poll_id=poll.id, choice_text=f"Choice {j+1}")
                db_session.add(choice)
        ...
        for poll in polls:
            for player in multiple_players:
                ...
                first_choice = (
                    db_session.query(PollChoice)
                    .filter(PollChoice.poll_id == poll.id)
                    .first()
                )

                vote = PollResponse(
                    poll_id=poll.id, choice_id=first_choice.id, player_id=player.id
                )
                db_session.add(vote)
        ...
        team_polls = db_session.query(Poll).filter(Poll.team_id == test_team.id).all()
...
class TestPollRegression:
    ...
    def test_poll_foreign_key_integrity(self, db_session, test_user, test_team):
        ...
        poll = Poll(
            team_id=test_team.id, created_by=test_user.id, question="Foreign key test?"
        )
    ...
    def test_cascade_deletion_behavior(
        ...
        poll = Poll(
            team_id=test_team.id, created_by=test_user.id, question="Cascade test?"
        )
        db_session.add(poll)
        db_session.flush()
        ...
        vote = PollResponse(
            poll_id=poll.id, choice_id=choice.id, player_id=test_player.id
        )
        db_session.add(vote)
        db_session.commit()
        ...
        deleted_choice = (
            db_session.query(PollChoice).filter(PollChoice.id == choice_id).first()
        )
        deleted_vote = (
            db_session.query(PollResponse)
    ...
    def test_duplicate_choice_prevention(self, db_session, test_user, test_team):
        ...
        choices = (
            db_session.query(PollChoice).filter(PollChoice.poll_id == poll.id).all()
        )
