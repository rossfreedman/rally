...
from app.models.database_models import Schedule, Team, League, Club, Series, User, Player
...
class TestScheduleAPI:
    ...
    def test_get_user_schedule(self, authenticated_client, db_session, test_user, test_player, test_team, test_league, linked_user_player):
    ...
    def test_schedule_page_rendering(self, authenticated_client, test_user):
        ...
        assert response.status_code == 200
        assert b'schedule' in response.data.lower()  # Check page contains schedule content

@pytest.mark.integration
class TestScheduleMatching:
    """Test schedule team matching logic"""
    
    def test_user_team_schedule_filtering(self, db_session, test_user, test_player, test_team, test_league, linked_user_player):
        ...
        user_schedules = db_session.query(Schedule).filter(
...
class TestScheduleUpdates:
    ...
    def test_schedule_modification(self, db_session, test_league):
        ...
        # Verify updates
        updated_schedule = db_session.query(Schedule).filter(Schedule.id == schedule_id).first()
...
class TestSchedulePerformance:
    ...
    def test_large_schedule_query_performance(self, db_session, test_league, test_team):
        ...
        user_schedules = db_session.query(Schedule).filter(
            (Schedule.home_team_id == test_team.id) |
            (Schedule.away_team_id == test_team.id)
        ).order_by(Schedule.match_date, Schedule.match_time).all()
...
class TestScheduleWithAvailability:
    ...
    def test_schedule_availability_integration(self, db_session, test_user, test_player, test_team, test_league, linked_user_player):
        ...
        schedule_with_availability = db_session.query(Schedule).filter(
            Schedule.match_date == match_date.date()
        ).first()
        
        player_availability = db_session.query(PlayerAvailability).filter(
...
class TestScheduleRegression:
    ...
    def test_duplicate_schedule_prevention(self, db_session, test_league, test_team):
        ...
        duplicate_count = db_session.query(Schedule).filter(
    ...
    def test_schedule_foreign_key_integrity(self, db_session, test_league, test_team):
        ...
        assert schedule in test_league.schedules if hasattr(test_league, 'schedules') else True
