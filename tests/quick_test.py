...
def run_security_tests():
    ...
    result = subprocess.run(
        ["pytest", "tests/test_security.py", "-v", "--tb=short"], capture_output=False
    )
    return result.returncode == 0
...
def run_schedule_tests():
    ...
    result = subprocess.run(
        ["pytest", "tests/test_schedule.py", "-v", "--tb=short"], capture_output=False
    )
    return result.returncode == 0
...
def run_polls_tests():
    ...
    result = subprocess.run(
        ["pytest", "tests/test_polls.py", "-v", "--tb=short"], capture_output=False
    )
    return result.returncode == 0
...
def scrape_fresh_data():
    ...
    result = subprocess.run(
        ["python", "tests/scrapers/random_league_scraper.py"], capture_output=False
    )
    return result.returncode == 0
...
def validate_test_data():
    ...
    scraped_file = fixtures_dir / "sampled_players.json"
    ...
    try:
        ...
        print(
            f"   🏆 Leagues: {', '.join(data['metadata'].get('leagues_scraped', []))}"
        )
        ...
        if response.status_code == 200:
            ...
            result = subprocess.run(
                [
                    "locust",
                    "-f",
                    "tests/load/load_test_registration.py",
                    "--host",
                    "http://localhost:8080",
                    "--users",
                    "5",
...
def check_test_environment():
    ...
    required_packages = {
        ...
        "psycopg2-binary": "psycopg2",  # Install name vs import name
    }
    missing_packages = []
...
def main():
    ...
    parser.add_argument(
        "--registration", action="store_true", help="Run registration tests"
    )
