...
from app.models.database_models import User, Player, UserPlayerAssociation
from app.services.auth_service_refactored import register_user, authenticate_user
...
class TestUserRegistration:
    ...
    def test_register_user_with_player_link(self, db_session, test_league, test_club, test_series, test_team):
        ...
        user = db_session.query(User).filter(User.email == 'linkeduser@example.com').first()
        assert user is not None
        
        # Verify player association was created
        association = db_session.query(UserPlayerAssociation).filter(
    ...
    def test_register_missing_required_fields(self, db_session, missing_field, field_value):
...
class TestPlayerLinking:
    ...
    def test_link_to_existing_player(self, db_session, test_league, test_club, test_series, scraped_test_data):
        ...
        user = db_session.query(User).filter(User.email == 'playerlink@example.com').first()
...
class TestRegistrationRegression:
    ...
    def test_player_association_integrity(self, db_session, test_league, test_club, test_series):
