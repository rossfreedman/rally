...
def test_club(db_session):
    """Create a test club"""
    club = Club(name="Test Club", club_address="123 Test St, Test City, TX 12345")
    db_session.add(club)
    db_session.commit()
    return club
...
def scraped_test_data():
    """Load test data sampled from local league JSON files"""
    test_data_file = os.path.join(
        os.path.dirname(__file__), "fixtures", "sampled_players.json"
    )
