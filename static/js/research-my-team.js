...
async function setUserTeamInfoMyTeam() {
    try {
        ...
        if (authData.authenticated && authData.user) {
            ...
            document.getElementById('userSeriesMyTeam').textContent = displaySeries || '';
...
async function renderTeamAnalysisCards(container) {
    ...
    const [matches, stats] = await Promise.all([
                    fetch('/data/leagues/all/match_history.json').then(r => r.json()),
            fetch('/data/leagues/all/series_stats.json').then(r => r.json())
    ]);
