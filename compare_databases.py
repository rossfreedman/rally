...
from database_config import get_db_url, get_db_engine
import logging
...
class DatabaseComparator:
    ...
    def _get_local_url(self):
        ...
        try:
            url = get_db_url()
            logger.info(f"Local database URL: {url.split('@')[1] if '@' in url else 'unknown'}")
            return url
        finally:
            if original_sync:
    ...
    def _get_railway_url(self):
        ...
        logger.info(f"Railway database URL: {url.split('@')[1] if '@' in url else 'unknown'}")
        return url
    ...
    def compare_schemas(self):
        ...
        for table in common_tables:
            ...
            railway_cols = {col['name']: col for col in railway_schema['tables'][table]['columns']}
        ...
        differences['summary'] = {
            ...
            'schema_identical': (
                len(differences['tables_only_in_local']) == 0 and
                len(differences['tables_only_in_railway']) == 0 and
                len(differences['column_differences']) == 0
            )
        }
    ...
    def compare_data(self):
        ...
        for table in common_tables:
            ...
            if local_count != railway_count:
                data_differences['row_count_differences'][table] = {
                    ...
                    'difference': local_count - railway_count if isinstance(local_count, int) and isinstance(railway_count, int) else 'N/A'
                }
        
        # Create summary
        total_local_rows = sum(count for count in local_counts.values() if isinstance(count, int))
        total_railway_rows = sum(count for count in railway_counts.values() if isinstance(count, int))
        ...
        data_differences['summary'] = {
            ...
            'tables_with_count_differences': len(data_differences['row_count_differences']),
    ...
    def generate_alembic_diff(self):
        ...
        try:
            ...
            logger.info("Comparing Railway → Local (changes needed for Railway to match Local)")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                revision_output = subprocess.run([
                    ...
                    "--message", "temp_comparison", "--head", "head"
    ...
    def run_full_comparison(self):
        ...
        try:
            ...
            report = {
                ...
                'databases': {
                    'local': self.local_url.split('@')[1] if '@' in self.local_url else 'unknown',
                    'railway': self.railway_url.split('@')[1] if '@' in self.railway_url else 'unknown'
                }
            }
    ...
    def print_report(self, report):
        ...
        print(f"Schema identical: {'✅ YES' if schema['schema_identical'] else '❌ NO'}")
        print()
        ...
        if not schema['schema_identical']:
            ...
            if report['schema_comparison']['tables_only_in_local']:
                print(f"Tables only in LOCAL: {report['schema_comparison']['tables_only_in_local']}")
            
            if report['schema_comparison']['tables_only_in_railway']:
                print(f"Tables only in RAILWAY: {report['schema_comparison']['tables_only_in_railway']}")
            
            if report['schema_comparison']['column_differences']:
                print("Column differences:")
                for table, diffs in report['schema_comparison']['column_differences'].items():
                    print(f"  {table}:")
                    if diffs['only_in_local']:
                        print(f"    Columns only in local: {diffs['only_in_local']}")
                    if diffs['only_in_railway']:
                        print(f"    Columns only in railway: {diffs['only_in_railway']}")
        ...
        if not data['data_identical']:
            ...
            if report['data_comparison']['row_count_differences']:
                ...
                for table, counts in report['data_comparison']['row_count_differences'].items():
                    print(f"  {table}: Local={counts['local_count']}, Railway={counts['railway_count']}, Diff={counts['difference']}")
            print()
