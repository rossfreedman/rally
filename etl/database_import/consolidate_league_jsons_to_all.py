...
# Navigate up three levels: etl/database_import -> etl -> project_root
BASE_DIR = Path(__file__).parent.parent.parent / "data" / "leagues"
...
def get_league_folders() -> List[Path]:
    ...
    for item in BASE_DIR.iterdir():
        if item.is_dir() and item.name != "all" and not item.name.startswith('.') and not item.name.startswith('backup_'):
...
def append_to_consolidated_file(filename: str, data: List[Dict[Any, Any]], league_name: str) -> None:
