...
7. Import player_history.json -> player_history table
8. Import match_history.json -> match_scores table
9. Import series_stats.json -> series_stats table
10. Import schedules.json -> schedule table
"""
...
class ComprehensiveETL:
    ...
    def import_players(self, conn, players_data: List[Dict]):
        ...
        for player in players_data:
            try:
                ...
                cursor.execute("""
                    ...
                    WHERE l.league_id = %s
                    ON CONFLICT ON CONSTRAINT unique_player_in_league DO UPDATE SET
                ...
                if imported % 1000 == 0:
                    self.log(f"   📊 Imported {imported:,} players so far...")
        ...
        conn.commit()
        self.imported_counts['players'] = imported
        self.log(f"✅ Imported {imported:,} players ({errors} errors)")
