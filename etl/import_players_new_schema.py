...
def import_players_from_json(json_file_path: str, dry_run: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Player import completed successfully!")
        ...
        # Import career stats after players are imported
        logger.info("Starting career stats import...")
        career_stats_result = import_career_stats_for_players(db_session)
        
        if career_stats_result['success']:
            logger.info(f"Career stats import completed: {career_stats_result['updated_count']} players updated")
            stats['career_stats_updated'] = career_stats_result['updated_count']
        else:
            logger.warning(f"Career stats import failed: {career_stats_result['error']}")
            stats['career_stats_error'] = career_stats_result['error']
...
def import_career_stats_for_players(db_session) -> Dict[str, Any]:
    """
    Import career stats from player_history.json for existing players
    """
    try:
        # Find player_history.json file
        possible_paths = [
        ...
        if not player_history_path:
            return {
                'success': False,
                'error': 'Could not find player_history.json file',
                'updated_count': 0
            }
        
        logger.info(f"Loading career stats from: {player_history_path}")
        
        # Load JSON data
        with open(player_history_path, 'r') as f:
            players_data = json.load(f)
        
        logger.info(f"Loaded {len(players_data)} player records from career stats JSON")
        ...
        for player_data in players_data:
            ...
            # Update all player records with this tenniscores_player_id
            players_updated = db_session.query(Player).filter(
                Player.tenniscores_player_id == tenniscores_id
            ).update({
                'career_wins': career_wins,
                'career_losses': career_losses,
                'career_matches': career_matches,
                'career_win_percentage': career_win_percentage,
                'updated_at': datetime.now()
            })
            
            if players_updated > 0:
                updated_count += players_updated
            else:
                not_found_count += 1
        
        # Commit the career stats updates
        db_session.commit()
        
        logger.info(f"Career stats import results:")
        logger.info(f"  - Player records updated: {updated_count}")
        logger.info(f"  - Players not found: {not_found_count}")
        logger.info(f"  - Players skipped (no stats): {skipped_no_stats}")
        
        return {
            'success': True,
            'updated_count': updated_count,
            'not_found_count': not_found_count,
            'skipped_count': skipped_no_stats
        }
        
    except Exception as e:
        logger.error(f"Error importing career stats: {str(e)}")
        return {
            'success': False,
            'error': str(e),
            'updated_count': 0
