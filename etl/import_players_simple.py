#!/usr/bin/env python3
"""
Simple ETL Script: Import Players to New Schema

This script imports player data from JSON files directly into the new schema
using raw SQL instead of ORM to avoid transaction issues.
"""
...
def import_players(json_file_path, dry_run=False):
    """Import players from JSON file with new schema support"""
    ...
    try:
        ...
        for i, player in enumerate(players_data, 1):
            ...
            try:
                ...
                league_name = player.get('League', '').strip()
                club_name = player.get('Club', '').strip()
                series_name = player.get('Series', '').strip()
                
                if not player_id or not first_name or not last_name:
                    print(f"   ⚠️  Skipping record {i}: Missing required fields")
                    continue
                
                # Get foreign key IDs (with caching)
                if league_name not in league_cache:
                    league_cache[league_name] = get_or_create_league(cursor, league_name)
                league_db_id = league_cache[league_name]
                ...
                pti = parse_numeric_value(player.get('PTI'))
                wins = int(parse_numeric_value(player.get('Wins', 0)) or 0)
                losses = int(parse_numeric_value(player.get('Losses', 0)) or 0)
                win_percentage = parse_numeric_value(player.get('Win %'))
                ...
                # Insert/update player record directly into new schema
                cursor.execute(
                    """
                    INSERT INTO players 
                    (tenniscores_player_id, first_name, last_name, league_id, club_id, series_id,
                     pti, wins, losses, win_percentage, captain_status, is_active, created_at, updated_at)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (tenniscores_player_id, league_id) DO UPDATE
                    ...
                    """,
                    (player_id, first_name, last_name, league_db_id, club_db_id, series_db_id,
                     pti, wins, losses, win_percentage, captain_status, True, datetime.now(), datetime.now())
                )
                result = cursor.fetchone()
                if result['created']:
                    players_created += 1
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Import players to new schema")
    parser.add_argument(
        '--file', 
