...
def run_master_scraper(league_subdomain):
    ...
    scraping_sequence = [
        ("Player Data Scraper", scrape_league_players, [league_subdomain], {"get_detailed_stats": True}),
    ...
    for scraper_num, (scraper_name, scraper_func, args, kwargs) in enumerate(scraping_sequence, 1):
    ...
    print(f"❌ Failed scrapers: {failed_scrapers}/{len(scraping_sequence)} ({(failed_scrapers/len(scraping_sequence))*100:.1f}%)")
    print(f"⚡ Average time per scraper: {format_duration(total_duration / len(scraping_sequence))}")
    print()
    ...
    if failed_scrapers == 0:
        ...
        print(f"🏆 Complete data collection for {league_subdomain.upper()} finished in {format_duration(overall_duration)}")
    else:
        print(f"⚠️  SCRAPING COMPLETED WITH {failed_scrapers} FAILURE(S)")
        print(f"📊 {successful_scrapers} out of {len(scraping_sequence)} scrapers completed successfully")
...
def main():
    ...
    print("=" * 70)
    print("🔍 This script will run ALL scrapers in sequence for comprehensive data collection")
    ...
    while True:
        ...
        if not validate_league_subdomain(league_subdomain):
            print("❌ Invalid league subdomain format. Please use alphanumeric characters only.")
            continue
    ...
    confirm = input("🚀 Ready to start complete data collection? (y/N): ").strip().lower()
    ...
    print(f"Master scraping {'completed successfully' if success else 'finished with some failures'} for {league_subdomain.upper()}")
