...
def fix_ross_freedman_linking():
    ...
    if matches:
        ...
        print(
            f"   SQL: UPDATE player_history SET player_id = {ross_player_id} WHERE end_pti = {ross_pti} AND series matches Chicago 22"
        )
        ...
        try:
            ...
            print(
                f"   ✅ Verification: Ross now has {verification['count']} linked PTI history records"
            )
...
def analyze_linking_opportunities():
    ...
    if opportunities:
        ...
        for opp in opportunities:
            print(
                f"     {opp['first_name']} {opp['last_name']} (PTI: {opp['pti']}): {opp['history_matches']} records"
            )
...
def implement_bulk_linking(opportunities, limit=5):
    ...
    for i, opp in enumerate(opportunities[:limit]):
        ...
        pattern_conditions = " OR ".join(
            [f"ph.series ILIKE '{pattern}'" for pattern in series_patterns]
        )
...
def verify_fixes():
    ...
    print(
        f"   📊 Linked records: {linked_count:,} / {total_count:,} ({percentage:.1f}%)"
    )
