...
def preview_migration_plan():
    ...
    with get_db() as local_conn:
        ...
        migrate_tables = [
            'players', 'match_scores', 'player_history', 'schedule', 
            'series_stats', 'player_availability', 'user_player_associations'
        ]
...
def test_migration_sample():
    ...
    test_tables = [
        'players', 'match_scores', 'player_history', 'schedule', 
        'series_stats', 'player_availability'
    ]
        ...
        for table in test_tables:
            try:
                ...
                if local_count == 0:
                    ...
                    test_results[table] = {'status': 'skipped', 'reason': 'no local data'}
                    continue
                ...
                railway_cursor.execute(f"CREATE TABLE {test_table} AS SELECT * FROM {table} LIMIT 0")
                
                # Get sample data from local (5 records)
                local_cursor.execute(f"SELECT * FROM {table} ORDER BY {columns[0]} LIMIT 5")
                sample_data = local_cursor.fetchall()
                
                if not sample_data:
                    logger.info(f"  📭 {table}: No sample data available")
                    test_results[table] = {'status': 'skipped', 'reason': 'no sample data'}
                    continue
                ...
                logger.info(f"  ✅ {table}: Successfully migrated {inserted_count} test records")
                test_results[table] = {
                    'status': 'success', 
...
def verify_current_railway_functionality():
    ...
    tests = [
        ...
        ("Leagues configured", "SELECT COUNT(*) FROM leagues WHERE league_id IN ('APTA_CHICAGO', 'NSTF')"),
