...
def check_local_missing_tables():
    ...
    with get_db() as conn:
        ...
        if ph_count > 0:
            # Sample records
            cursor.execute('SELECT * FROM player_history ORDER BY created_at DESC LIMIT 3')
            print(f'  Sample records:')
            for row in cursor.fetchall():
                print(f'    ID: {row[0]}, Player ID: {row[1]}, League ID: {row[2]}, Series: {row[3]}, Date: {row[4]}, PTI: {row[5]}')
            
            # Check for NULL player_id values
            cursor.execute('SELECT COUNT(*) FROM player_history WHERE player_id IS NULL')
        ...
        if upa_count > 0:
            # Sample records
            cursor.execute('SELECT * FROM user_player_associations ORDER BY created_at DESC LIMIT 5')
            print(f'  Sample records:')
            for row in cursor.fetchall():
                print(f'    User ID: {row[0]}, Player ID: {row[1]}, Primary: {row[2]}, Created: {row[3]}')
