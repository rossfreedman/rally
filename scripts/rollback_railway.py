...
def verify_restore_success(pre_restore_counts):
    ...
    for table in sorted(all_tables):
        ...
        logger.info(
            f"{table:<25} {str(before_count):<15} {str(after_count):<15} {status:<15}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rollback Railway database from backup"
    )
    ...
    if pre_restore_counts:
        total_records = sum(
            count for count in pre_restore_counts.values() if isinstance(count, int)
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python scripts/rollback_railway.py railway_backup_20241201_143022.dump"
        )
        sys.exit(1)
