...
class DataQualityMonitor:
    ...
    def calculate_basic_metrics(self):
        ...
        # Core table counts
        tables = ['leagues', 'clubs', 'series', 'teams', 'players', 'match_scores', 'series_stats', 'schedule']
        
        for table in tables:
            try:
                count_result = execute_query_one(f"SELECT COUNT(*) as count FROM {table}")
    ...
    def calculate_data_freshness(self):
        ...
        try:
            ...
            if latest_match and latest_match['latest_date']:
                ...
                if days_since > 14:
                    self.add_alert('WARNING', 'Stale match data', f"Latest match is {days_since} days old")
                if recent_count < 10:
                    self.add_alert('WARNING', 'Low recent activity', f"Only {recent_count} matches in past 7 days")
    ...
    def calculate_completeness_metrics(self):
        ...
        try:
            ...
            if player_completeness:
                ...
                name_rate = (player_completeness['has_first_name'] / total * 100) if total > 0 else 0
                club_rate = (player_completeness['has_club'] / total * 100) if total > 0 else 0
                id_rate = (player_completeness['has_player_id'] / total * 100) if total > 0 else 0
                ...
                if name_rate < 90:
                    self.add_alert('WARNING', 'Poor player name data', f"Only {name_rate:.1f}% have names")
                if id_rate < 80:
                    self.add_alert('WARNING', 'Poor player ID linkage', f"Only {id_rate:.1f}% have Tennis ID")
        ...
        try:
            ...
            if match_quality:
                ...
                winner_rate = (match_quality['has_winner'] / total * 100) if total > 0 else 0
                score_rate = (match_quality['has_scores'] / total * 100) if total > 0 else 0
                team_rate = (match_quality['has_teams'] / total * 100) if total > 0 else 0
                ...
                if winner_rate < 95:
                    self.add_alert('WARNING', 'Missing match winners', f"Only {winner_rate:.1f}% have winners")
                if team_rate < 99:
                    self.add_alert('ERROR', 'Missing team data', f"Only {team_rate:.1f}% have complete team info")
    ...
    def calculate_consistency_metrics(self):
        ...
        try:
            ...
            if stats_consistency:
                ...
                print(f"✅ Series stats coverage: {coverage:.1f}% ({stats_teams}/{match_teams} teams)")
                
                if coverage < 90:
                    self.add_alert('ERROR', 'Poor series stats coverage', f"Only {coverage:.1f}% teams have statistics")
                elif coverage < 95:
                    self.add_alert('WARNING', 'Incomplete series stats', f"{coverage:.1f}% coverage")
        ...
        try:
            ...
            if integrity_check:
                ...
                if orphaned_players > 0:
                    self.add_alert('WARNING', 'Orphaned player records', f"{orphaned_players} players without valid league")
                if orphaned_stats > 0:
                    self.add_alert('ERROR', 'Orphaned series stats', f"{orphaned_stats} stats without valid league")
                if orphaned_matches > 0:
                    self.add_alert('ERROR', 'Orphaned matches', f"{orphaned_matches} matches without valid league")
        ...
        for league_id in major_leagues:
            try:
                ...
                if league_health:
                    ...
                    print(f"✅ {league_id}: {players:,} players, {teams} teams, {matches:,} matches")
                    ...
                    thresholds = min_thresholds.get(league_id, {'players': 50, 'teams': 10, 'matches': 100})
                    
                    if players < thresholds['players']:
                        self.add_alert('WARNING', f'{league_id} low player count', f"Only {players} players (need {thresholds['players']})")
                    if teams < thresholds['teams']:
                        self.add_alert('WARNING', f'{league_id} low team count', f"Only {teams} teams (need {thresholds['teams']})")
                    if matches < thresholds['matches']:
                        self.add_alert('WARNING', f'{league_id} low match count', f"Only {matches} matches (need {thresholds['matches']})")
                        
                    # Check data staleness
                    if latest:
                        days_since = (datetime.now().date() - latest).days
                        if days_since > 30:
                            self.add_alert('WARNING', f'{league_id} stale data', f"Latest match is {days_since} days old")
                else:
                    self.add_alert('ERROR', f'League {league_id} not found', 'League missing from database')
        ...
        try:
            ...
            if perf_metrics:
                ...
                if db_size_mb > 5000:  # 5GB threshold
                    self.add_alert('WARNING', 'Large database size', f"Database is {db_size_mb:,} MB")
                if active_conns > 20:
                    self.add_alert('WARNING', 'High connection count', f"{active_conns} active connections")
    ...
    def print_monitoring_report(self):
        ...
        for label, metric_key in key_metrics:
            ...
            if isinstance(value, (int, float)) and metric_key.endswith(('_completeness', '_coverage')):
                print(f"   📊 {label}: {value}%")
