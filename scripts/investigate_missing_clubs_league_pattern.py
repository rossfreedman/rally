    ...
    with get_db() as local_conn:
        ...
        for club_id in missing_club_ids:
            ...
            if club_info:
                ...
                for league_id, league_name, series_id, series_name, count in league_series_data:
                    print(f"  • League {league_id} ({league_name}) - Series {series_id} ({series_name}): {count} players")
    ...
    with get_db() as local_conn:
        ...
        for series_id, series_name in series_2b:
            ...
            for club_id, club_name, player_count in clubs_in_series_2b:
                status = "❌ MISSING" if club_id in [94, 91, 10, 93, 45, 44, 92, 97, 26, 47, 48, 27] else "✅ MAPPED"
                print(f"      • {club_name} (ID: {club_id}): {player_count} players - {status}")
...
def analyze_nsft_league_pattern():
    ...
    with get_db() as local_conn:
        ...
        # Find NSFT league
        local_cursor.execute("SELECT id, league_id, league_name FROM leagues WHERE league_name ILIKE '%NSFT%' OR league_id ILIKE '%NSFT%'")
        ...
        if nsft_leagues:
            ...
            local_cursor.execute("SELECT id, league_id, league_name FROM leagues WHERE id = 3")
            ...
            if league_3:
                ...
                print(f"  • Missing percentage: {(total_missing / (total_missing + total_mapped) * 100):.1f}%")
...
def cross_reference_with_railway():
    ...
    with get_db() as local_conn:
        ...
        railway_cursor.execute("SELECT id, name FROM series WHERE name ILIKE '%2B%' ORDER BY name")
        ...
        for league_id, league_name, club_count, player_count in railway_league_dist:
            print(f"  • League {league_id} ({league_name}): {club_count} clubs, {player_count} players")
