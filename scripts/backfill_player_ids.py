    ...
    for i in range(0, len(users), batch_size):
        ...
        for user in batch:
            ...
            try:
                ...
                if player_id:
                    logger.info(
                        f"Found match for {first_name} {last_name}: {player_id}"
                    )
                    ...
                    if not dry_run:
                        ...
                        if success:
                            ...
                            logger.error(
                                f"Failed to update database for {first_name} {last_name}"
                            )
                            errors += 1
                            error_users.append(
                                {
                    ...
                    logger.info(
                        f"No match found for {first_name} {last_name} ({club_name}, {series_name})"
                    )
                    no_matches += 1
...
def print_detailed_results(results: Dict, section: str = None):
    ...
    sections_to_print = (
        [section] if section else ["updated_users", "no_match_users", "error_users"]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Backfill Tenniscores Player IDs for existing Rally users"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--show-section",
        choices=["updated", "no_match", "errors"],
