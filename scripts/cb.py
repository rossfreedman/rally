...
def list_backups():
    ...
    try:
        ...
        if os.path.exists(codebase_backup_dir):
            codebase_backups = [d for d in os.listdir(codebase_backup_dir) 
                              if os.path.isdir(os.path.join(codebase_backup_dir, d)) and d.startswith('rally_')]
        ...
        if os.path.exists(db_backup_dir):
            ...
            for i, backup in enumerate(db_backups[:5], 1):  # Show 5 most recent
                ...
                print(f"{i}. {backup['name']} (created: {created_str}, size: {size_mb:.2f} MB)")
                print(f"   Format: {format_info}, Alembic: {alembic_rev[:20]}{'...' if len(str(alembic_rev)) > 20 else ''}")
        ...
        print(f"\n📊 BACKUP SUMMARY")
        print("-" * 30)
        codebase_count = len(codebase_backups) if os.path.exists(codebase_backup_dir) else 0
...
def cleanup_old_backups(backup_dir, max_backups, no_confirm=False):
    ...
    try:
        ...
        if len(backups) > max_backups:
            ...
            if not no_confirm:
                confirm = input(f"Delete {len(backups_to_delete)} old codebase backup(s)? [y/N]: ")
                if confirm.lower() != 'y':
                    print("Skipping cleanup of old codebase backups.")
                    return
            
            print(f"🔄 Cleaning up {len(backups_to_delete)} old codebase backups (keeping {max_backups} most recent)...")
            ...
            print(f"Codebase backup cleanup complete. {len(backups_to_delete)} old backups removed.")
...
def create_backup(max_backups=10, exclude_patterns=None, no_confirm=False, 
                 codebase_only=False, database_only=False, db_format='sql'):
    """
    ...
    try:
        ...
        if not codebase_only:
            database_backup_path = create_database_backup(db_backup_dir, db_format, max_backups)
...
def cleanup_old_database_backups(backup_dir, max_backups, no_confirm=False):
    ...
    try:
        ...
        for item in os.listdir(backup_dir):
            if item.startswith('rally_db_backup_') and not item.endswith('.metadata.json'):
        ...
        if len(backups) > max_backups:
            ...
            if not no_confirm:
                confirm = input(f"Delete {len(backups_to_delete)} old database backup(s)? [y/N]: ")
                if confirm.lower() != 'y':
            ...
            print(f"Database backup cleanup complete. {len(backups_to_delete)} old backups removed.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create backups of Rally codebase and/or database')
    ...
    parser.add_argument('--codebase-only', action='store_true',
    ...
    parser.add_argument('--db-format', choices=['sql', 'custom', 'tar'], 
                        default='sql', help='Database backup format (default: sql)')
