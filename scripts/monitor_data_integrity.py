    ...
    def check_availability_integrity(self):
        ...
        if orphaned_records:
            self.add_issue(
                ...
                examples=[f"ID:{r['id']} player_id:{r['player_id']} name:{r['player_name']}" for r in orphaned_records[:5]]
            )
        ...
        if name_id_mismatches:
            self.add_warning(
                ...
                examples=[f"ID:{r['id']} stored:'{r['player_name']}' actual:'{r['actual_name']}'" for r in name_id_mismatches[:5]]
            )
        ...
        if duplicates:
            self.add_issue(
                ...
                examples=[f"player_id:{r['player_id']} date:{r['match_date']} series:{r['series_id']} ({r['duplicate_count']} records)" for r in duplicates[:5]]
            )
        ...
        if orphaned_series:
            self.add_issue(
                ...
                examples=[f"ID:{r['id']} name:{r['player_name']} series_id:{r['series_id']}" for r in orphaned_series[:5]]
            )
    ...
    def check_players_integrity(self):
        ...
        if players_no_series:
            self.add_issue(
                ...
                examples=[f"ID:{r['id']} {r['first_name']} {r['last_name']} series_id:{r['series_id']}" for r in players_no_series[:5]]
            )
        ...
        if duplicate_players:
            self.add_warning(
                ...
                examples=[f"{r['first_name']} {r['last_name']} series:{r['series_id']} ({r['duplicate_count']} records)" for r in duplicate_players[:5]]
            )
    ...
    def generate_report(self):
        ...
        if self.issues:
            ...
            for i, issue in enumerate(self.issues, 1):
                ...
                if issue['examples']:
                    print(f"   Examples: {issue['examples'][:3]}")  # Show up to 3 examples
                print()
        ...
        if any(issue['type'] == 'NULL_PLAYER_ID' for issue in self.issues):
            print("• Run scripts/fix_availability_player_ids_COMPLETED.py to fix NULL player_id values")
        
        if any(issue['type'] == 'ORPHANED_RECORDS' for issue in self.issues):
            print("• Clean up orphaned availability records referencing non-existent players")
        ...
        if self.issues:
            print("• Run migrations/enforce_player_id_integrity.sql to add database constraints")
            print("• Consider implementing automated data validation in the application")
