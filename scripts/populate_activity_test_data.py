...
def create_activity_entries():
    ...
    for day_offset in range(30):
        ...
        num_activities = (
            random.randint(5, 25) if day_offset < 7 else random.randint(2, 15)
        )
        ...
        for _ in range(num_activities):
            ...
            timestamp = activity_date.replace(
            ...
            player = (
                random.choice(players) if players and random.random() < 0.7 else None
            )
            ...
            if action_type == "login":
                action_description = (
                    f"User {user['first_name']} {user['last_name']} logged in"
                )
            elif action_type == "match_created":
                action_description = (
                    f"Match result created for {team['team_name'] if team else 'team'}"
                )
            elif action_type == "poll_response":
                action_description = (
                    f"{player['first_name']} {player['last_name']} responded to team poll"
                    if player
                    else "Team poll response received"
                )
            elif action_type == "availability_update":
                statuses = ["available", "unavailable", "maybe"]
                status = random.choice(statuses)
                action_description = (
                    f"{player['first_name']} {player['last_name']} marked as {status}"
                    if player
                    else f"Availability updated to {status}"
                )
    ...
    try:
        total_count = execute_query_one("SELECT COUNT(*) as count FROM activity_log")[
            "count"
        ]
...
def main():
    ...
    while True:
        clear_choice = input(
            "Do you want to clear existing activity data first? (y/n): "
        ).lower()
