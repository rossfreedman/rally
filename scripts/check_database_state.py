...
def compare_databases():
    ...
    logger.info("-" * 40)
    logger.info(
        f"Local   - Database: {local_info['database']}, User: {local_info['user']}"
    )
    logger.info(
        f"Railway - Database: {railway_info['database']}, User: {railway_info['user']}"
    )
    ...
    for table in sorted(common_tables):
        ...
        logger.info(
            f"{table:<25} {str(local_count):<15} {str(railway_count):<15} {status:<15}"
        )
    ...
    if total_differences == 0:
        ...
        logger.warning(
            f"📊 Total record difference: {abs(local_total - railway_total):,}"
        )
