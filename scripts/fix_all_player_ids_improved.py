...
def load_player_history() -> List[Dict]:
    ...
    player_history_path = os.path.join(
        project_root, "data", "leagues", "APTA", "player_history.json"
    )
...
def find_player_by_name(
    players: List[Dict], first_name: str, last_name: str, club: str = None
) -> Optional[Dict]:
    ...
    for first_var in first_variations:
            ...
            if normalize_name(player.get("name", "")) == target_name:
                ...
                if club and player.get("club"):
                    if normalize_name(player.get("club", "")) == normalize_name(club):
                        print(
                            f"   ✅ Found exact match with club: {player['name']} at {player['club']}"
                        )
                        return player
                else:
        ...
        if club:
            for player in players:
                if normalize_name(player.get("name", "")) == target_name:
                    print(
                        f"   ✅ Found match (different club): {player['name']} at {player.get('club', 'Unknown')}"
                    )
                    return player
...
def fix_all_player_ids():
    ...
    print(
        "🔍 This version handles name variations like Jon/Jonathan, Mike/Michael, etc."
    )
    print("=" * 70)
    ...
    for user in users:
        ...
        matching_player = find_player_by_name(
            players, user["first_name"], user["last_name"], club_name
        )

        if not matching_player:
            print(
                f"   ❌ No matching player found in player history (even with name variations)"
            )
            stats["not_found"] += 1
            continue
    ...
    success_rate = (
        (stats["already_correct"] + stats["updated"]) / stats["total_users"]
    ) * 100
