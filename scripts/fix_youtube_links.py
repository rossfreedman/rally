        ...
        try:
            ...
            for item in search_response["items"]:
                ...
                score = calculate_video_score(
                    title, description, technique_name, search_term
                )
...
def fix_youtube_links_enhanced():
    ...
    # Load the JSON file
    json_file = (
        "data/leagues/all/improve_data/complete_platform_tennis_training_guide.json"
    )
    ...
    for technique_name, technique_data in data.items():
        ...
        if (
            not isinstance(technique_data, dict)
            or "Reference Videos" not in technique_data
        ):
            continue
        ...
        if new_url:
            ...
            if reference_videos:
                ...
                technique_data["Reference Videos"] = [
                    {"title": f"{technique_name} Training Video", "url": new_url}
                ]

            updated_count += 1
            techniques_processed.append(
                {"name": technique_name, "url": new_url, "source": source}
            )
...
def generate_update_report(techniques_processed, total_updated):
    ...
    with open(report_file, "w") as f:
        ...
        f.write("\n## Next Steps\n\n")
        f.write(
            "1. **Test Videos**: Verify that key technique videos are appropriate\n"
        )
        f.write(
            "2. **Manual Curation**: Replace any generic videos with technique-specific ones\n"
        )
        f.write(
            "3. **API Enhancement**: Set up YouTube Data API for better search results\n"
        )
        f.write(
            "4. **Regular Updates**: Run this script periodically to ensure links remain active\n"
        )
...
def manual_curation_helper():
    ...
    for technique_name, technique_data in data.items():
        ...
        if any(
            priority.lower() in technique_name.lower()
            for priority in PRIORITY_TECHNIQUES
        ):
            priority_found = True
            reference_videos = technique_data.get("Reference Videos", [])
            current_url = (
                reference_videos[0].get("url", "") if reference_videos else "No URL"
            )
    ...
    if not priority_found:
        ...
        for technique_name in data.keys():
            if (
                isinstance(data[technique_name], dict)
                and "Reference Videos" in data[technique_name]
            ):
