...
scrapers_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'etl', 'scrapers')
...
def test_stealth_browser():
    ...
    try:
        ...
        with StealthBrowserManager(headless=True, max_retries=2) as driver:
            ...
            driver.get("data:text/html,<html><body><h1>Stealth Test Success!</h1></body></html>")
            ...
            if "Chrome" in page_source:
                print("✅ Successfully navigated to real website with Chrome user agent")
            else:
        ...
        print("💡 Make sure undetected-chromedriver is installed: pip install undetected-chromedriver")
        return False
    ...
    for scraper_name in scrapers_to_test:
        try:
            ...
            if hasattr(scraper_module, 'StealthBrowserManager'):
                ...
                print(f"   ⚠️ {scraper_name} may not have imported StealthBrowserManager correctly")
