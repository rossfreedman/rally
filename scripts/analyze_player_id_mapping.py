    ...
    with get_db() as local_conn:
        ...
        try:
            ...
            for player in local_players:
                print(
                    f"    ID: {player[0]}, TC_ID: {player[1]}, Name: {player[2]} {player[3]}, PTI: {player[4]}"
                )
            ...
            if local_problem_player:
                print(
                    f"  Local player ID 11748: {local_problem_player[3]} {local_problem_player[4]} (TC: {local_problem_player[1]})"
                )
            ...
            for player_id, count in ph_player_refs:
                local_cursor.execute(
                    "SELECT first_name, last_name, tenniscores_player_id FROM players WHERE id = %s",
                    (player_id,),
                )
                player_info = local_cursor.fetchone()
                if player_info:
                    print(
                        f"    Player ID {player_id}: {count:,} records - {player_info[0]} {player_info[1]} (TC: {player_info[2]})"
                    )
                else:
                    print(
                        f"    Player ID {player_id}: {count:,} records - PLAYER NOT FOUND"
                    )
            ...
            if 11748 in local_to_railway_mapping:
                print(
                    f"  ✅ Player ID 11748 maps to Railway ID: {local_to_railway_mapping[11748]}"
                )
            else:
