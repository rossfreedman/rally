...
def show_current_status():
    ...
    if os.path.exists(json_file):
        ...
        try:
            ...
            # Count videos
            total_videos = sum(
                len(tech.get("Reference Videos", [])) for tech in data.values()
            )
            techniques_with_videos = sum(
                1 for tech in data.values() if tech.get("Reference Videos")
            )
            print(
                f"📹 Current videos: {total_videos} across {techniques_with_videos} techniques"
            )
