...
def final_migrate():
    ...
    try:
        ...
        try:
            ...
            new_cursor.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS club_automation_password VARCHAR(255);")
            new_cursor.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS last_login TIMESTAMP;")
            new_cursor.execute("ALTER TABLE users DROP COLUMN IF EXISTS phone;")  # Remove column that doesn't exist in old
            new_conn.commit()
        ...
        try:
            ...
            new_cursor.execute("ALTER TABLE player_availability RENAME COLUMN availability_status TO availability;")
            new_cursor.execute("ALTER TABLE player_availability RENAME COLUMN updated_at TO submitted_at;")
            new_conn.commit()
...
def verify_final(old_cursor, new_cursor, new_conn):
    ...
    tables = ['alembic_version', 'clubs', 'series', 'users', 'player_availability', 'user_activity_logs', 'user_instructions']
