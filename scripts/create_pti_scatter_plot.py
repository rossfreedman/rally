...
def find_player_in_players_json(
    players_data: List[Dict], player_name: str
) -> Optional[float]:
...
def create_scatter_plot_data(player_name: str, data_dir: str = "data") -> pd.DataFrame:
    ...
    for match in matches:
        ...
        if not corresponding_match:
            print(
                f"Warning: Could not find match record for {player_name} on {match_date_str}"
            )
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create PTI vs Opponent Analysis Scatter Plot"
    )
    parser.add_argument(
        "--player", default="Daniel Sullivan", help="Player name to analyze"
    )
    parser.add_argument(
        "--data-dir", default="data", help="Directory containing JSON data files"
    )
    ...
    print(
        f"Player PTI range: {df['player_end_pti'].min():.1f} - {df['player_end_pti'].max():.1f}"
    )
    print(
        f"Opponent PTI range: {df['opponent_avg_pti'].min():.1f} - {df['opponent_avg_pti'].max():.1f}"
    )
