...
class SkippedPlayersAnalyzer:
    ...
    def load_existing_mappings(self):
        ...
        railway_series_by_name = {
            name.strip().lower(): id for id, name in railway_series
        }
        ...
        logger.info(
            f"  ✅ Loaded {len([v for v in self.series_mapping.values() if v is not None])} series mappings"
        )
    ...
    def analyze_all_players(self):
        ...
        logger.info(
            f"  ✅ Analysis complete: {len(migrated_players)} migrated, {len(skipped_players)} skipped"
        )
    ...
    def categorize_skipped_players(self, skipped_players):
        ...
        for player in skipped_players:
            ...
            if has_club_issue and has_series_issue:
                ...
                self.skipped_analysis["skip_reasons"]["unmapped_club_only"].append(
                    player
                )
            elif has_series_issue:
                self.skipped_analysis["skip_reasons"]["unmapped_series_only"].append(
                    player
                )
            else:
    ...
    def analyze_unmapped_entities(self, skipped_players):
        ...
        logger.info(
            f"  📊 Found {len(self.skipped_analysis['unmapped_clubs'])} unmapped clubs"
        )
        logger.info(
            f"  📊 Found {len(self.skipped_analysis['unmapped_series'])} unmapped series"
        )
    ...
    def analyze_data_quality(self, skipped_players):
        ...
        self.skipped_analysis["data_quality_analysis"] = {
            ...
            "most_affected_series": dict(
                sorted(
                    [
                        (series, len(players))
                        for series, players in series_patterns.items()
                    ],
                    key=lambda x: x[1],
                    reverse=True,
                )[:10]
    ...
    def generate_recommendations(self):
        ...
        if unmapped_clubs:
            total_players_affected_by_clubs = sum(
                club["player_count"] for club in unmapped_clubs.values()
            )
            top_clubs = sorted(
                unmapped_clubs.values(), key=lambda x: x["player_count"], reverse=True
            )[:5]
            ...
            recommendations.append(
                {
                    "type": "club_mapping",
                    "priority": (
                        "HIGH" if total_players_affected_by_clubs > 200 else "MEDIUM"
                    ),
                    ...
                    "top_impact_clubs": [
                        f"{club['name']} ({club['player_count']} players)"
        ...
        if unmapped_series:
            total_players_affected_by_series = sum(
                series["player_count"] for series in unmapped_series.values()
            )
            top_series = sorted(
                unmapped_series.values(), key=lambda x: x["player_count"], reverse=True
            )[:3]
            ...
            recommendations.append(
                {
                    "type": "series_mapping",
                    "priority": (
                        "HIGH" if total_players_affected_by_series > 100 else "MEDIUM"
                    ),
                    ...
                    "top_impact_series": [
                        f"{series['name']} ({series['player_count']} players)"
    ...
    def print_executive_summary(self):
        ...
        print(
            f"  • Unmapped Club Issues: {len(skip_reasons['unmapped_club_only']) + len(skip_reasons['both_unmapped'])}"
        )
        print(
            f"  • Unmapped Series Issues: {len(skip_reasons['unmapped_series_only']) + len(skip_reasons['both_unmapped'])}"
        )
        ...
        for reason, players in skip_reasons.items():
            if players:
                ...
                print(
                    f"  • {reason.replace('_', ' ').title()}: {len(players)} ({percentage:.1f}%)"
                )
        ...
        club_recovery = sum(
            club["player_count"]
            for club in self.skipped_analysis["unmapped_clubs"].values()
        )
        series_recovery = sum(
            series["player_count"]
            for series in self.skipped_analysis["unmapped_series"].values()
        )
        ...
        print(
            f"  • Total potential recovery: {min(club_recovery + series_recovery, total)} players"
        )
        print(
            f"  • Potential final coverage: {((2431 + min(club_recovery + series_recovery, total)) / 2964 * 100):.1f}%"
        )
