...
def get_railway_url(url):
    ...
    if parsed.hostname == "postgres.railway.internal":
        ...
        parts[1] = parts[1].replace(
            "postgres.railway.internal", "postgres-production-0931.up.railway.app"
        )
...
def sync_table(source_conn, dest_conn, table_name, primary_key="id"):
    ...
    with source_conn.cursor() as source_cur, dest_conn.cursor() as dest_cur:
        ...
        dest_conn.commit()
        print(
            f"✅ Synced {table_name}: {len(to_insert)} inserts, {len(to_update)} updates, {len(to_delete)} deletes"
        )
...
def main():
    ...
    try:
        ...
        print(
            "Railway URL should use 'postgres-production-0931.up.railway.app' as the hostname."
        )
