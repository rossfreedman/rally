...
def migrate_player_history_simple(local_conn, railway_conn, mapping):
    ...
    local_cursor.execute("SELECT COUNT(*) FROM player_history WHERE player_id IN %s", (tuple(mapping.keys()),))
    ...
    try:
        with get_db() as local_conn:
            ...
            try:
                ...
                # Migrate tables
                ph_count = migrate_player_history_simple(local_conn, railway_conn, mapping)
                upa_count = migrate_user_associations_simple(local_conn, railway_conn, mapping)
