...
def map_players_to_database_ids(json_data):
    ...
    for player_data in json_data:
        ...
        recent_match = max(
            matches,
            key=lambda m: parse_date(m.get("date", "1/1/1900"))
        ...
        if pti_key in players_by_pti:
            ...
            if best_match:
                ...
                enhanced_mappings += 1
                print(
                    f"   ✅ Enhanced mapping: {player_data.get('name', 'Unknown')} -> {best_match['first_name']} {best_match['last_name']} (PTI: {end_pti})"
                )

    print(
        f"   ✅ Created {enhanced_mappings:,} additional mappings via PTI+series matching"
    )
...
def import_player_history_records(json_data, player_id_map, league_id_map):
    ...
    if records_to_insert:
        ...
        try:
            ...
            print(
                f"   ✅ Successfully inserted {len(records_to_insert):,} player_history records"
            )
...
def verify_import():
    ...
    if ross_records:
        ...
        for record in ross_records:
            print(
                f"     {record['formatted_date']}: PTI={record['end_pti']} - {record['series']}"
            )
    else:
...
def test_mobile_service():
    ...
    try:
        ...
        if result.get("pti_data_available"):
            print(
                f"   ✅ Mobile service working with proper player_history foreign keys!"
            )
        else:
...
def main():
    """Main function to orchestrate the reimport process"""
    print("🔄 REIMPORTING PLAYER_HISTORY WITH CORRECT PLAYER_IDS")
    print("=" * 60)
    ...
    try:
        ...
        # Step 3: Map players to database IDs
        ...
        imported_count = import_player_history_records(
            json_data, player_id_map, league_id_map
        )
