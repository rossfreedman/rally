...
class DailyIntegrityMonitor:
    ...
    def check_availability_integrity(self):
        ...
        if null_player_ids['count'] > 0:
            self.add_alert('ALERT', 'AVAILABILITY', 
        ...
        if invalid_status['count'] > 0:
            self.add_alert('WARNING', 'AVAILABILITY',
        ...
        return {
            'total_records': execute_query_one("SELECT COUNT(*) as count FROM player_availability")['count'],
    ...
    def check_player_integrity(self):
        ...
        if players_no_series['count'] > 0:
            self.add_alert('WARNING', 'PLAYERS',
        ...
        return {
            'total_players': execute_query_one("SELECT COUNT(*) as count FROM players WHERE is_active = true")['count'],
    ...
    def check_constraint_health(self):
        ...
        for constraint in critical_constraints:
            if constraint not in existing_names:
                self.add_alert('ALERT', 'CONSTRAINTS',
                              f"Critical constraint missing: {constraint}")
        ...
        for index in critical_indexes:
            if index not in existing_index_names:
                self.add_alert('WARNING', 'INDEXES',
                              f"Performance index missing: {index}")
    ...
    def run_daily_check(self):
        ...
        try:
            ...
            print(f"   Player availability records: {availability_stats['total_records']}")
            print(f"   Recent availability updates (7d): {availability_stats['recent_activity']}")
            ...
            if self.warnings:
                ...
                for warning in self.warnings:
                    count_str = f" ({warning['count']} records)" if warning['count'] else ""
                    print(f"   ⚠️  [{warning['category']}] {warning['message']}{count_str}")
