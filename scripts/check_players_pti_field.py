    ...
    if ross_data['pti'] is not None:
        ...
        if series_data:
            ...
            for record in series_data:
                print(f"     {record['formatted_date']}: PTI={record['end_pti']}, Series={record['series']}")
...
if other_players:
    ...
    for player in other_players:
        print(f"     {player['first_name']} {player['last_name']}: PTI={player['pti']} (ID: {player['tenniscores_player_id']})")
else:
...
if ross_data and ross_data['series_id']:
    ...
    if series_info:
        ...
        patterns = [series_name, f"%{series_name}%", f"{series_name}%", f"%{series_name}"]
        
        for pattern in patterns:
            series_records = execute_query(series_history_query, [pattern])
            if series_records:
                print(f"   Found {len(series_records)} records matching pattern '{pattern}':")
