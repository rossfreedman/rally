...
def print_detailed_results(results: Dict, section: str = None):
    ...
    sections_to_print = (
        [section]
        if section
        else ["newly_matched_users", "no_match_users", "missing_data_users"]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Audit Rally user to Tenniscores Player ID matching"
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
    ...
    parser.add_argument(
        "--show-section",
        choices=["newly_matched", "no_match", "missing_data"],
