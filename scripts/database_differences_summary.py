...
def main():
    ...
    with get_db() as local_conn:
        ...
        print(
            f"{'Table':<25} {'Local':<12} {'Railway':<12} {'Difference':<12} {'Status':<15}"
        )
        print("-" * 80)
            ...
            try:
                ...
                if railway_count == local_count:
                    ...
                    partial_migrations.append(
                        (table, local_count, railway_count, coverage)
                    )
                else:
                    status = f"🟢 {coverage:.1f}%"

                print(
                    f"{table:<25} {local_count:<12} {railway_count:<12} {diff:<12} {status:<15}"
                )

            except Exception as e:
                print(
                    f"{table:<25} {'ERROR':<12} {'ERROR':<12} {'N/A':<12} {'❌ ERROR':<15}"
                )
        ...
        if partial_migrations:
            ...
            for table, local, railway, coverage in partial_migrations:
                ...
                print(
                    f"  • {table}: {missing:,} records missing ({coverage:.1f}% coverage)"
                )
        ...
        total_local_records = (
            sum(local_count for table, local_count in perfect_matches)
            + sum(local for _, local, _, _ in partial_migrations)
            + sum(count for _, count in critical_gaps)
        )
        total_railway_records = sum(
            local_count for table, local_count in perfect_matches
        ) + sum(railway for _, _, railway, _ in partial_migrations)

        overall_coverage = (
            (total_railway_records / total_local_records * 100)
            if total_local_records > 0
            else 0
        )
        ...
        if missing_in_railway:
            print(
                f"  2. MEDIUM: Add {len(missing_in_railway)} missing clubs to Railway"
            )
