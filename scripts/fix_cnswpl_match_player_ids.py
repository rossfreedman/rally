...
def lookup_player_id_enhanced(players_data, series, team_name, first_name, last_name):
    ...
    print(
        f"      Looking for: '{first_name}' '{last_name}' in '{players_series}' at '{club_name}'"
    )

    # Strategy 1: Exact match (Series + Full Team Name + Names)
    for player in players_data:
        if (
            player.get("Series") == players_series
            ...
            print(
                f"      ✅ Strategy 1 match: {player.get('First Name')} {player.get('Last Name')} → {player.get('Player ID')}"
            )
    ...
    if len(series_matches) == 1:
        ...
        print(
            f"      ✅ Strategy 2 match: {player.get('First Name')} {player.get('Last Name')} → {player.get('Player ID')} (Club: {player.get('Club')})"
        )
        return player.get("Player ID")
    ...
    for player in players_data:
        if player.get("Series") == players_series and player.get("Club") == club_name:
            ...
            if first_match and last_match:
                print(
                    f"      ✅ Strategy 3 match: {player.get('First Name')} {player.get('Last Name')} → {player.get('Player ID')}"
                )
                return player.get("Player ID")
    ...
    for player in players_data:
        if player.get("Series") == players_series and player.get("Club") == club_name:
            ...
            if first_similar and last_similar:
                print(
                    f"      ✅ Strategy 4 match: {player.get('First Name')} {player.get('Last Name')} → {player.get('Player ID')}"
                )
                return player.get("Player ID")
...
def fix_cnswpl_match_player_ids(league_data_dir=None, verbose=True):
    ...
    for match_idx, match in enumerate(matches):
        ...
        for player_name_key, player_id_key, team_name in player_positions:
            ...
            if show_debug:
                print(
                    f"    🔍 Processing: {player_name} (Team: {team_name} → Series: {series_name})"
                )

            # Lookup player ID (suppress output unless in debug mode)
            if show_debug:
                player_id = lookup_player_id_enhanced(
                    players_data, series_name, team_name, first_name, last_name
                )
            elif verbose:
            ...
            if player_id:
                ...
                if (
                    stats["fixed_players"] <= 10 or show_debug
                ) and verbose:  # Show first 10 fixes or debug matches
