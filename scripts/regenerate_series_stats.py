...
def update_series_stats_table(team_stats, team_series_map):
    ...
    for team_name, stats in team_stats.items():
        ...
        execute_query(insert_query, [
            series_name, team_name, team_id, stats['points'],
            stats['matches_won'], stats['matches_lost'], stats['matches_tied'],
            stats['lines_won'], stats['lines_lost'], 0, 0,  # lines_for, lines_ret
...
def verify_tennaqua_12():
    ...
    if tennaqua_stats:
        ...
        print(f"      Matches: {tennaqua_stats['matches_won']}-{tennaqua_stats['matches_lost']}")
        print(f"      Lines: {tennaqua_stats['lines_won']}-{tennaqua_stats['lines_lost']}")
        print(f"      Sets: {tennaqua_stats['sets_won']}-{tennaqua_stats['sets_lost']}")
        print(f"      Games: {tennaqua_stats['games_won']}-{tennaqua_stats['games_lost']}")
    else:
