...
def test_team_schedule():
    ...
    if login_response.status_code == 200:
        ...
        if page_response.status_code == 200:
            ...
            if (
                "team-schedule" in page_response.text
                and "loadTeamScheduleData" in page_response.text
            ):
                print("✅ Page appears to contain team schedule functionality")
            else:
                print(
                    "⚠️  Page loaded but may not contain expected team schedule elements"
                )
        else:
        ...
        if api_response.status_code == 200:
            try:
                ...
                if players_schedule:
                    print("\n👥 Sample players:")
                    for i, (player_name, availability) in enumerate(
                        list(players_schedule.items())[:3]
                    ):
                        print(f"   {i+1}. {player_name} - {len(availability)} dates")
                        if availability:
                            sample_date = availability[0]
                            print(
                                f"      Sample: {sample_date.get('date')} - Status: {sample_date.get('availability_status')} - Type: {sample_date.get('event_type')}"
                            )
                ...
                if event_details:
                    ...
                    for i, (date, details) in enumerate(
                        list(event_details.items())[:3]
                    ):
                        print(
                            f"   {date}: {details.get('type')} - {details.get('opponent', details.get('description', 'N/A'))}"
                        )
