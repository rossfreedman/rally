...
def fix_availability_player_ids():
    ...
    for record in null_records:
        print(
            f"     ID {record['id']}: {record['player_name']} (series {record['series_id']}) - {record['match_date']}"
        )
    ...
    for record in null_records:
        ...
        matching_player = execute_query_one(
            player_lookup_query, (player_name, series_id)
        )
        ...
        if matching_player:
            ...
            print(
                f"   ✅ {player_name} -> player_id {matching_player['id']} ({matching_player['series_name']})"
            )
        else:
            not_found.append(
                {
            ...
            print(
                f"   ❌ {player_name} -> No matching player found in series {series_id}"
            )
        ...
        for fix in fixes:
            try:
                rows_affected = execute_update(
                    update_query, (fix["player_id"], fix["availability_id"])
                )
                if rows_affected > 0:
                    fixes_applied += 1
                    print(
                        f"   ✅ Fixed: {fix['player_name']} -> player_id {fix['player_id']}"
                    )
                else:
        ...
        for record in not_found:
            ...
            if similar_players:
                ...
                for similar in similar_players:
                    similar_full_name = (
                        f"{similar['first_name']} {similar['last_name']}"
                    )
