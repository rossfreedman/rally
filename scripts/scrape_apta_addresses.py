...
class APTAAddressScraper:
    ...
    def extract_address_from_team_page(self, team_info):
        ...
        try:
            ...
            directions_links = soup.find_all('a', href=True, string=re.compile(r'get\s+directions', re.I))
...
def update_club_address(club_name, address):
    ...
    try:
        success = execute_update(
            "UPDATE clubs SET club_address = %s WHERE name = %s",
            (address, club_name)
        )
...
def main():
    ...
    for i, team_info in enumerate(team_links, 1):
        ...
        matching_db_clubs = [c['name'] for c in db_clubs if c['name'].lower() == club_name_lower]
