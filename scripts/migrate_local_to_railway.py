...
def migrate_table_data(local_conn, railway_conn, table_name, strategy="replace", batch_size=1000):
...
def migrate_essential_data():
    ...
    with get_db() as local_conn:
        ...
        for table_name, strategy in tables_to_migrate:
            try:
                migrated_count = migrate_table_data(local_conn, railway_conn, table_name, strategy)
                migration_summary['migrated_tables'].append(f"{table_name}: {migrated_count} records")
        ...
        logger.info(f"\n🔧 UPDATING SEQUENCES...")
        sequence_tables = ['players', 'match_scores', 'player_history', 'schedule', 'series_stats', 'player_availability']
        ...
        logger.info(f"✅ MIGRATED TABLES ({len(migration_summary['migrated_tables'])}):")
