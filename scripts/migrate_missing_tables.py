...
def migrate_player_history(local_conn, railway_conn):
    ...
    while offset < total_local:
        ...
        batch_data = [(row[1], row[2], row[3], row[4], row[5], row[6]) for row in batch_rows]
...
def migrate_user_player_associations(local_conn, railway_conn):
    ...
    for local_user_id, local_player_id, is_primary, created_at, user_email, tc_player_id in local_associations:
        # Map user ID
