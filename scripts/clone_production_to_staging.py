...
class ProductionStagingCloner:
    ...
    def check_pg_tools(self):
        ...
        for tool in required_tools:
            try:
                ...
                raise Exception(
                    f"Required tool {tool} is not installed. Please install PostgreSQL client tools."
                )
    ...
    def get_database_stats(self, db_url: str, db_name: str) -> dict:
        ...
        try:
            ...
            stats = {"table_count": table_count, "row_count": row_count}
    ...
    def clone_production_to_staging(self):
        ...
        try:
            ...
            result = subprocess.run(
                dump_cmd, capture_output=True, text=True, timeout=600
            )
            ...
            result = subprocess.run(
                restore_cmd, capture_output=True, text=True, timeout=600
            )
    ...
    def verify_clone_success(self):
        ...
        if prod_stats["row_count"] == staging_stats["row_count"]:
            ...
            logger.warning(
                "⚠️  Row counts don't match (this may be normal due to timing)"
            )
