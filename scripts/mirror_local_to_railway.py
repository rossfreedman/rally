...
def backup_railway_database(backup_file):
    ...
    try:
        ...
        cmd = [
            "pg_dump",
            "-h",
            RAILWAY_DB_CONFIG["host"],
            "-p",
            str(RAILWAY_DB_CONFIG["port"]),
            "-U",
            RAILWAY_DB_CONFIG["user"],
            "-d",
...
def sync_alembic_migration_state():
    ...
    try:
        ...
        if local_migration and local_migration != "None":
            ...
            revision = (
                local_migration.split()[0]
                if " " in local_migration
                else local_migration
            )
            result = subprocess.run(
                ["alembic", "stamp", revision], env=env, capture_output=True, text=True
            )
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"❌ Failed to set Railway migration state: {result.stderr}"
                )
                return False
        else:
...
def verify_mirror_success():
    ...
    for table in sorted(all_tables):
        ...
        if local_count == railway_count:
            logger.info(
                f"✅ {table:25} Local: {local_count:8,} | Railway: {railway_count:8,}"
            )
        else:
            logger.error(
                f"❌ {table:25} Local: {local_count:8,} | Railway: {railway_count:8,}"
    ...
    if success:
        ...
        logger.info(
            f"Total records - Local: {local_total:,} | Railway: {railway_total:,}"
        )
...
def main():
    ...
    parser.add_argument(
        "--skip-backup", action="store_true", help="Skip Railway backup step"
    )
    ...
    logger.info("-" * 30)
    response = input(
        "This will REPLACE ALL Railway data with local data. Continue? (yes/no): "
    )
    ...
    if not args.skip_backup:
        ...
        backup_file = os.path.join(
            "data", "backups", "database", f"railway_backup_{timestamp}.dump"
        )
