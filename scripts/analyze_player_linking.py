...
print(f"   Unlinked records: {history_count - linked_count:,} records ({((history_count - linked_count) / history_count * 100):.1f}%)")
...
if series_matches:
    ...
    for match in series_matches:
        print(f"       {match['series_name']}: {match['players_in_series']} players, {match['history_series_variants']} history variants")
...
if pti_correlations:
    ...
    for corr in pti_correlations:
        print(f"       {corr['first_name']} {corr['last_name']} (PTI: {corr['current_pti']}): {corr['matching_history_records']} history records")
...
if ross_data:
    ...
    for variant in series_variants:
        ...
        if series_matches:
            ...
            for match in series_matches:
                print(f"       ID {match['id']}: {match['formatted_date']} - PTI: {match['end_pti']}")
...
if ross_data and exact_matches:
    ...
    print(f"     Link player_history.id = {best_match['id']} to players.id = {ross_data['id']}")
    print(f"     Record: {best_match['formatted_date']} - PTI: {best_match['end_pti']} - Series: {best_match['series']}")
