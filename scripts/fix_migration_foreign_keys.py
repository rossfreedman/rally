...
def create_club_mapping():
    ...
    with get_db() as local_conn:
        ...
        railway_clubs = {name: railway_id for railway_id, name in railway_cursor.fetchall()}
        ...
        for name, local_id in local_clubs.items():
            if name in railway_clubs:
                ...
                logger.info(f"  ✅ {name}: local_id {local_id} → railway_id {railway_clubs[name]}")
            else:
                missing_clubs.append((local_id, name))
                logger.warning(f"  ❌ Missing in Railway: {name} (local_id: {local_id})")
...
def create_series_mapping():
    ...
    with get_db() as local_conn:
        ...
        railway_series = {name: railway_id for railway_id, name in railway_cursor.fetchall()}
        ...
        for name, local_id in local_series.items():
            if name in railway_series:
                ...
                logger.info(f"  ✅ {name}: local_id {local_id} → railway_id {railway_series[name]}")
            else:
    ...
    with get_db() as local_conn:
        ...
        for local_id, club_name in missing_clubs:
            ...
            if club_data:
                ...
                logger.info(f"  ✅ Added {name}: local_id {local_id} → new railway_id {new_railway_id}")
    ...
    with get_db() as local_conn:
        ...
        for player_row in players:
            ...
            if original_club_id in club_mapping:
                ...
                logger.warning(f"  ⚠️  Skipping player {player_dict['first_name']} {player_dict['last_name']}: unmapped club_id {original_club_id}")
                skipped_count += 1
                continue
            ...
            if original_series_id in series_mapping:
                ...
                logger.warning(f"  ⚠️  Skipping player {player_dict['first_name']} {player_dict['last_name']}: unmapped series_id {original_series_id}")
                skipped_count += 1
                continue
            ...
            try:
                ...
                logger.error(f"  ❌ Failed to migrate player {player_dict['first_name']} {player_dict['last_name']}: {e}")
                skipped_count += 1
        
        railway_conn.commit()
        railway_conn.close()
        
        logger.info(f"  ✅ Players migration completed: {migrated_count} migrated, {skipped_count} skipped")
    ...
    with get_db() as local_conn:
        ...
        for table_name in tables_to_retry:
            try:
                ...
                railway_conn.commit()
                logger.info(f"  ✅ {table_name}: Migration completed ({migrated_count} records)")
...
def main():
    ...
    player_count = migrate_players_with_mapping(complete_club_mapping, complete_series_mapping)
    
    # Step 5: Complete remaining migrations
    complete_remaining_migrations()
    
    logger.info(f"\n✅ MIGRATION FIX COMPLETED!")
    logger.info(f"🎯 Railway now has your complete dataset with proper foreign key relationships")
