...
class ETLPipelineValidator:
    ...
    def create_test_database(self):
        ...
        try:
            ...
            self.test_db_url = self.original_db_url.replace(
                db_params["dbname"], self.test_db_name
            )
            return True
    ...
    def clone_schema_only(self):
        ...
        try:
            ...
            restore_proc = subprocess.run(
                restore_cmd, input=dump_proc.stdout, env=env, capture_output=True
            )
            ...
            with psycopg2.connect(**db_params) as conn:
                with conn.cursor() as cursor:
                    ...
                    if "players" in self.validation_results["source_data"]:
                        ...
                        if db_leagues >= expected_leagues:
                            completeness_results.append(
                                f"✅ Leagues: {db_leagues}/{expected_leagues} imported"
                            )
                        else:
                            completeness_results.append(
                                f"❌ Leagues: {db_leagues}/{expected_leagues} imported"
                        ...
                        if db_clubs >= expected_clubs:
                            completeness_results.append(
                                f"✅ Clubs: {db_clubs}/{expected_clubs} imported"
                            )
                        else:
                            completeness_results.append(
                                f"❌ Clubs: {db_clubs}/{expected_clubs} imported"
                        ...
                        if db_series >= expected_series:
                            completeness_results.append(
                                f"✅ Series: {db_series}/{expected_series} imported"
                            )
                        else:
                            completeness_results.append(
                                f"❌ Series: {db_series}/{expected_series} imported"
                        ...
                        if (
                            db_players >= expected_players * 0.95
                        ):  # Allow 5% tolerance for duplicates/errors
                            completeness_results.append(
                                f"✅ Players: {db_players}/{expected_players} imported"
                            )
                        else:
                            completeness_results.append(
                                f"❌ Players: {db_players}/{expected_players} imported"
                    ...
                    if "player_history" in self.validation_results["source_data"]:
                        source = self.validation_results["source_data"][
                            "player_history"
                        ]
                        ...
                        if db_history >= expected_history * 0.9:
                            completeness_results.append(
                                f"✅ Player History: {db_history}/{expected_history} records imported"
                            )
                        else:
                            completeness_results.append(
                                f"❌ Player History: {db_history}/{expected_history} records imported"
            ...
            with psycopg2.connect(**db_params) as conn:
                with conn.cursor() as cursor:
                    ...
                    for test in integrity_tests:
                        ...
                        if violations == 0:
                            integrity_results.append(
                                f"✅ {test['name']}: No violations"
                            )
                        else:
                            integrity_results.append(
                                f"❌ {test['name']}: {violations} violations"
                            )
                            overall_pass = False
                    ...
                    if empty_names == 0:
                        ...
                        integrity_results.append(
                            f"⚠️ {empty_names} players missing first names"
                        )
                    ...
                    if total_history > 0:
                        ...
                        if link_percentage >= 95:
                            integrity_results.append(
                                f"✅ Player history linking: {link_percentage:.1f}%"
                            )
                        else:
                            integrity_results.append(
                                f"⚠️ Player history linking: {link_percentage:.1f}% (below 95%)"
                            )
    ...
    def generate_report(self):
        ...
        print("📋 Test Results:")
        print(
            f"   Script Reliability: {'✅ PASS' if test_results['script_reliability']['status'] == 'pass' else '❌ FAIL'}"
        )
        print(
            f"   JSON Completeness:  {'✅ PASS' if test_results['json_completeness']['status'] == 'pass' else '❌ FAIL'}"
        )
        print(
            f"   Data Integrity:     {'✅ PASS' if test_results['data_integrity']['status'] == 'pass' else '❌ FAIL'}"
        )
        ...
        if "players" in source_data:
            ...
            print(
                f"   Reference Data: {players['league_count']} leagues, {players['club_count']} clubs, {players['series_count']} series"
            )

        if "player_history" in source_data:
            history = source_data["player_history"]
            print(
                f"   Player History: {history['total_match_records']:,} match records"
            )
    ...
    def validate(self):
        ...
        try:
            ...
            completeness_pass = (
                self.test_json_completeness() if reliability_pass else False
            )
...
def main():
    ...
    parser.add_argument(
        "--test-db-name", default="rally_etl_validation", help="Name for test database"
    )
