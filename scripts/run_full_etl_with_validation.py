...
def validate_json_data():
    ...
    validation_script = os.path.join(
        project_root, "data/etl/validation/winner_validation.py"
    )
    success = run_command(
...
def run_database_import():
    ...
    import_script = os.path.join(
        project_root, "data/etl/database_import/import_all_jsons_to_database.py"
    )
    success = run_command(
        f"python {import_script}", "Database import with winner validation"
    )
...
🎯 SPECIFIC VALIDATIONS:
   ...
   """
        ...
        report_path = os.path.join(
            project_root,
            "logs",
            f"etl_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run complete ETL pipeline with validation"
    )
    parser.add_argument(
        "--scrape", action="store_true", help="Run fresh scraping before import"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Only run validation, skip scraping and import",
    )
    parser.add_argument(
        "--skip-validation",
        action="store_true",
        help="Skip pre-import validation checks",
    )
