...
def compare_table_schemas(table_name):
    ...
    with get_db() as local_conn:
        ...
        """)
        local_schema = {row[0]: {'type': row[1], 'nullable': row[2], 'default': row[3]} 
        ...
        railway_cursor.execute(f"""
            ...
            ORDER BY ordinal_position
        """)
        railway_schema = {row[0]: {'type': row[1], 'nullable': row[2], 'default': row[3]} 
        ...
        for column_info in problematic_columns:
            ...
            try:
                ...
                if column_info['railway_type'] == 'boolean':
                    print(f"  🔧 Conversion needed: {column_info['local_type']} → boolean")
                    if isinstance(results[0][0], str):
                        print(f"    Suggestion: Map string values to boolean")
                        print(f"    Example mapping: 'C' → true, '' → false, NULL → NULL")
...
def fix_schema_mismatches():
    ...
    tables_to_check = ['players', 'match_scores', 'player_history', 'schedule', 'series_stats']
    ...
    if all_mismatches:
        ...
        for table, mismatch in all_mismatches:
            ...
            if railway_type == 'boolean' and local_type in ['character varying', 'text']:
                print(f"  Fix: Convert Railway column to TEXT or map local data to boolean")
                print(f"  SQL: ALTER TABLE {table} ALTER COLUMN {column} TYPE TEXT;")
            elif local_type == 'boolean' and railway_type in ['character varying', 'text']:
                print(f"  Fix: Convert local boolean values to strings during migration")
            else:
                print(f"  Fix: Manual review required for {local_type} → {railway_type}")
...
if __name__ == "__main__":
    ...
    if mismatches:
        ...
        logger.info(f"🔧 Review the suggested fixes above before proceeding with migration")
    else:
