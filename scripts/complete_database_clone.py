                ...
                for table in tables:
                    try:
                        ...
                        if count > 0:
                            ...
                            while offset < count:
                                cursor.execute(f"SELECT * FROM {table} ORDER BY {columns[0]} LIMIT {batch_size} OFFSET {offset}")
                                rows = cursor.fetchall()
        ...
        for i, command in enumerate(sql_commands):
            ...
            if command and not command.startswith('--'):
                try:
                    ...
                    if executed % 1000 == 0:
                        conn.commit()
                        print(f"    ✅ Executed {executed:,}/{len(sql_commands):,} commands")
...
def verify_clone():
    ...
    try:
        ...
        critical_tables = [
            'users', 'players', 'player_history', 'user_player_associations',
            'leagues', 'clubs', 'series'
        ]
