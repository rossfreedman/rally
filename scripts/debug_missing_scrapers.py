...
sys.path.append(
    os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "etl", "scrapers")
)
    ...
    try:
        ...
        if result:
            ...
            for player in result[:3]:  # Show first 3 players
                print(
                    f"   - {player.get('First Name', 'N/A')} {player.get('Last Name', 'N/A')} ({player.get('Series', 'N/A')})"
                )
        else:
...
def check_file_outputs():
    ...
    cnswpl_dir = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", "leagues", "CNSWPL"
    )
