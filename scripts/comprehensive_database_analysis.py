...
class DatabaseAnalyzer:
    ...
    def analyze_table_counts(self):
        ...
        for table in tables:
            ...
            self.analysis_results['table_counts'][table] = {
                ...
                'difference': railway_count - local_count if isinstance(local_count, int) and isinstance(railway_count, int) else 'N/A',
                'coverage_percent': (railway_count / local_count * 100) if isinstance(local_count, int) and isinstance(railway_count, int) and local_count > 0 else 0
            }
        
        print("\n" + "="*100)
        print("📊 TABLE COUNTS COMPARISON")
        print("="*100)
        print(f"{'Table':<25} {'Local':<12} {'Railway':<12} {'Difference':<12} {'Coverage':<12} {'Status':<15}")
        print("-" * 100)
        ...
        for table, counts in self.analysis_results['table_counts'].items():
            ...
            coverage = f"{counts['coverage_percent']:.1f}%" if isinstance(counts['coverage_percent'], (int, float)) else "N/A"
            ...
            print(f"{table:<25} {str(local):<12} {str(railway):<12} {str(diff):<12} {coverage:<12} {status:<15}")
    ...
    def analyze_content_differences(self):
        ...
        for table in key_tables:
            ...
            try:
                ...
                table_analysis['local_sample'] = [dict(zip(local_columns, row)) for row in local_sample]
                ...
                table_analysis['railway_sample'] = [dict(zip(railway_columns, row)) for row in railway_sample]
                
                # For clubs and series, analyze unique entries
                if table in ['clubs', 'series']:
                    # Get all names
                    local_cursor.execute(f"SELECT name FROM {table}")
                    local_names = {row[0].strip().lower() for row in local_cursor.fetchall()}
                    
                    railway_cursor.execute(f"SELECT name FROM {table}")
                    railway_names = {row[0].strip().lower() for row in railway_cursor.fetchall()}
                    
                    table_analysis['unique_to_local'] = list(local_names - railway_names)
                    table_analysis['unique_to_railway'] = list(railway_names - local_names)
        ...
        for db_name, data in league_analysis.items():
            ...
            if isinstance(data, list):
                for league_info in data:
                    print(f"  • {league_info['league']}: {league_info['players']} players, {league_info['clubs']} clubs")
            else:
                print(f"  ❌ {data}")
    ...
    def analyze_club_series_relationships(self):
        ...
        for db_name, conn in [('local', self.local_conn), ('railway', self.railway_conn)]:
            cursor = conn.cursor()
        ...
        self.analysis_results['data_quality']['club_series_relationships'] = relationship_analysis
    ...
    def analyze_data_quality_issues(self):
        ...
        for db_name, conn in [('local', self.local_conn), ('railway', self.railway_conn)]:
            ...
            try:
                ...
                cursor.execute("SELECT COUNT(*) FROM players WHERE last_name IS NULL OR last_name = ''")
        ...
        for db_name, issues in quality_issues.items():
            ...
            if 'error' in issues:
                ...
                print(f"  • Players missing first name: {issues['players_missing_first_name']}")
                print(f"  • Players missing last name: {issues['players_missing_last_name']}")
                print(f"  • Players missing club: {issues['players_missing_club']}")
                print(f"  • Players missing series: {issues['players_missing_series']}")
                print(f"  • Orphaned club references: {issues['orphaned_club_references']}")
                
                if issues['duplicate_player_names']:
                    print(f"  • Top duplicate names:")
                    for name_info in issues['duplicate_player_names'][:5]:
                        print(f"    - {name_info[0]} {name_info[1]}: {name_info[2]} instances")
    ...
    def analyze_migration_status(self):
        ...
        # By league
        for league_id, league_name in [(1, 'APTA Chicago'), (2, 'APTA National'), (3, 'NSFT')]:
            local_cursor.execute("SELECT COUNT(*) FROM players WHERE league_id = %s", (league_id,))
            local_count = local_cursor.fetchone()[0]
            
            railway_cursor.execute("SELECT COUNT(*) FROM players WHERE league_id = %s", (league_id,))
            railway_count = railway_cursor.fetchone()[0]
            
            migration_stats[league_name] = {
                'local': local_count,
                'railway': railway_count,
                'migration_rate': (railway_count / local_count * 100) if local_count > 0 else 0
            }
        ...
        for league, stats in migration_stats.items():
            ...
            status = "🟢 EXCELLENT" if rate > 90 else "🟡 GOOD" if rate > 70 else "🔴 NEEDS WORK"
            print(f"  • {league}: {stats['railway']}/{stats['local']} ({rate:.1f}%) {status}")
    ...
    def generate_recommendations(self):
        ...
        for table, counts in table_counts.items():
            if isinstance(counts['coverage_percent'], (int, float)):
                ...
                if coverage == 0 and isinstance(counts['local'], int) and counts['local'] > 0:
                ...
                elif coverage < 50 and isinstance(counts['local'], int) and counts['local'] > 100:
                    recommendations.append({
                        'priority': 'MEDIUM',
    ...
    def print_executive_summary(self):
        ...
        total_local = sum(counts['local'] for counts in table_counts.values() if isinstance(counts['local'], int))
        total_railway = sum(counts['railway'] for counts in table_counts.values() if isinstance(counts['railway'], int))
        ...
        player_coverage = (players_railway / players_local * 100) if players_local > 0 else 0
        
        print(f"\n🏃 PLAYER MIGRATION STATUS:")
        print(f"  • Players migrated: {players_railway:,}/{players_local:,} ({player_coverage:.1f}%)")
        
        # Critical issues
        critical_issues = []
        for table, counts in table_counts.items():
            if isinstance(counts['coverage_percent'], (int, float)) and counts['coverage_percent'] == 0 and isinstance(counts['local'], int) and counts['local'] > 0:
        ...
        # Success metrics
        complete_tables = [table for table, counts in table_counts.items() 
                          if isinstance(counts['coverage_percent'], (int, float)) and counts['coverage_percent'] == 100]
