...
def import_career_stats():
    ...
    response = (
        input("Do you want to continue with this deprecated script? (y/N): ")
        .lower()
        .strip()
    )
    ...
    for player_data in players_data:
        ...
        try:
            ...
            if career_matches >= 20:
                ...
                print(
                    f"✅ Updated {db_name}: {career_wins}W-{career_losses}L ({career_win_percentage}%)"
                )
    ...
    try:
        ...
        for player in sample_players:
            print(
                f"   {player['first_name']} {player['last_name']}: {player['career_wins']}W-{player['career_losses']}L ({player['career_win_percentage']}%)"
            )
    except:
        pass
