...
# Search for Jeff Day
jeff_matches = [p for p in players if 'jeff' in p['First Name'].lower() or 'day' in p['Last Name'].lower()]
print(f'🔍 Players with "Jeff" or "Day" in name ({len(jeff_matches)} found):')
for p in jeff_matches[:10]:  # Show first 10
    print(f'  • {p["First Name"]} {p["Last Name"]} ({p["Club"]}) - {p["Series"]} - ID: {p["Player ID"]}')

print()

# Search for Scott Osterman  
scott_matches = [p for p in players if 'scott' in p['First Name'].lower() or 'osterman' in p['Last Name'].lower()]
...
pete_matches = [p for p in players if 'pete' in p['First Name'].lower() or 'wahlstrom' in p['Last Name'].lower() or 'peter' in p['First Name'].lower()]
print(f'🔍 Players with "Pete", "Peter" or "Wahlstrom" in name ({len(pete_matches)} found):')
