...
for col in columns:
    print(
        f"   {col['column_name']:15} {col['data_type']:15} {'NULL' if col['is_nullable'] == 'YES' else 'NOT NULL'}"
    )
...
if sample_records:
    for i, record in enumerate(sample_records):
        print(
            f"   Record {i+1}: player_id={record.get('player_id')}, date={record.get('date')}, end_pti={record.get('end_pti')}"
        )
else:
