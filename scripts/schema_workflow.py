...
class SchemaWorkflow:
    ...
    def sync_schema(self, auto_apply: bool = False) -> bool:
        ...
        if not status["alembic_initialized"]:
            logger.error(
                "❌ Alembic not initialized. Run 'python scripts/schema_sync_manager.py --init-baseline' first"
            )
            return False
        ...
        has_differences = (
            differences["summary"]["tables_missing_in_db"] > 0
            or differences["summary"]["tables_missing_in_models"] > 0
            or bool(differences["column_differences"])
        )
        ...
        if auto_apply:
            ...
            logger.info(
                "💡 Migration created. Review it and run 'alembic upgrade head' to apply"
            )
            return True
    ...
    def pre_commit_check(self) -> bool:
        ...
        differences = status["schema_differences"]
        has_model_changes = differences["summary"]["tables_missing_in_db"] > 0 or bool(
        ...
        if has_model_changes:
            ...
            logger.error(
                "Run 'python scripts/schema_workflow.py --create-migration' to create a migration"
            )
            return False
    ...
    def validate_models(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully imported {len(Base.metadata.tables)} model tables"
            )
            ...
            for table_name, table in Base.metadata.tables.items():
                ...
                for column in table.columns:
                    if column.foreign_keys and not any(
                        fk.constraint for fk in column.foreign_keys
                    ):
                        logger.warning(
                            f"⚠️  Column {table_name}.{column.name} has invalid foreign key"
                        )
...
def main():
    ...
    parser.add_argument(
        "--status", action="store_true", help="Show comprehensive status"
    )
    parser.add_argument(
        "--create-migration", metavar="MESSAGE", help="Create new migration"
    )
    parser.add_argument(
        "--apply-migrations", action="store_true", help="Apply pending migrations"
    )
    parser.add_argument("--sync", action="store_true", help="Complete sync workflow")
    parser.add_argument(
        "--sync-auto",
        action="store_true",
        help="Complete sync workflow with auto-apply",
    )
    parser.add_argument(
        "--pre-commit", action="store_true", help="Pre-commit hook check"
    )
    ...
    try:
        if args.status:
            ...
            print("=" * 30)
            print(
                f"Alembic Initialized: {'✅' if status['alembic_initialized'] else '❌'}"
            )
            print(f"Current Revision: {status['current_revision'] or 'None'}")
            print(
                f"Migrations Current: {'✅' if status['migrations_current'] else '❌'}"
            )
            print(
                f"Tables in Sync: {status['schema_differences']['summary']['tables_in_sync']}"
            )

            if status["schema_differences"]["column_differences"]:
                print(
                    f"Column Differences: {len(status['schema_differences']['column_differences'])} tables"
                )
            else:
