...
if matches:
    ...
    for i, match in enumerate(matches):
        print(
            f'  Match {i+1}: {match["Date"]} - {match["Home Team"]} vs {match["Away Team"]} - Winner: {match["Winner"]}'
        )
        ...
        if tenniscores_player_id in [match["Home Player 1"], match["Home Player 2"]]:
            position = (
                "Home Player 1"
                if match["Home Player 1"] == tenniscores_player_id
                else "Home Player 2"
            )
            partner = (
                match["Home Player 2"]
                if match["Home Player 1"] == tenniscores_player_id
                else match["Home Player 1"]
            )
            print(f"    Position: {position}, Partner: {partner}")
        else:
            position = (
                "Away Player 1"
                if match["Away Player 1"] == tenniscores_player_id
                else "Away Player 2"
            )
            partner = (
                match["Away Player 2"]
                if match["Away Player 1"] == tenniscores_player_id
                else match["Away Player 1"]
            )
            print(f"    Position: {position}, Partner: {partner}")
...
for date, date_matches in matches_by_date.items():
    ...
    if len(date_matches) > 1:
        print(
            f"    Multiple matches on same date - this could indicate court assignment"
        )
    else:
