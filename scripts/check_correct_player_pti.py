...
if player_lookup:
    ...
    if pti_records:
        ...
        for i, record in enumerate(pti_records):
            print(
                f'  {i+1:2d}. {record["formatted_date"]}: PTI = {record["end_pti"]:.1f}'
            )
        ...
        if other_players:
            print(
                f'   Other players with PTI data: {[p["player_id"] for p in other_players]}'
            )
    ...
    if similar_players:
        ...
        for p in similar_players:
            print(
                f'   {p["tenniscores_player_id"]}: {p["first_name"]} {p["last_name"]}'
            )
