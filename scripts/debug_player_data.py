...
def main():
    ...
    players = execute_query('SELECT * FROM players WHERE tenniscores_player_id = %s', [player_id])
    print('=== PLAYERS TABLE ===')
    if players:
        for player in players:
            print(f"ID: {player['id']}, Name: {player['first_name']} {player['last_name']}, League ID: {player.get('league_id', 'N/A')}")
    else:
    ...
    print('=== MATCH COUNT ===')
    print(f'Player {player_id} has {matches[0]["match_count"]} matches in match_scores table')
    print()
    
    # Check if this player exists in the users table
    users = execute_query('SELECT * FROM users WHERE tenniscores_player_id = %s', [player_id])
    print('=== USERS TABLE ===')
    if users:
        for user in users:
            print(f'ID: {user["id"]}, Name: {user["first_name"]} {user["last_name"]}, Email: {user["email"]}, Player ID: {user["tenniscores_player_id"]}')
    else:
        print("No users found with this player ID")
    print()
    
    # Check Ross Freedman's user record
    ross_users = execute_query('SELECT * FROM users WHERE email = %s', ['rossfreedman@gmail.com'])
