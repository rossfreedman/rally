...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("club_automation.log")],
)
...
def get_club_automation_credentials(email):
    ...
    with get_db() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT club_automation_password FROM users WHERE email = %s", (email,)
            )
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "#component_paddletennis_location_id")
                )
            )
