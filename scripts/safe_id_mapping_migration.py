...
class IDMappingAnalyzer:
    ...
    def analyze_club_mapping(self):
        ...
        logger.info(
            f"  📊 Local clubs: {len(local_clubs)}, Railway clubs: {len(railway_clubs)}"
        )
        ...
        for local_id, local_name in local_clubs:
            ...
            if name_key in railway_by_name:
                ...
                logger.warning(
                    f"  ❌ Missing in Railway: {local_name} (local_id: {local_id})"
                )
    ...
    def _find_alternate_club_matches(self, local_clubs, railway_clubs):
        ...
        missing_clubs = [
            detail
            for detail in self.mapping_report["clubs"]["details"]
        ...
        for missing in missing_clubs:
            ...
            if potential_matches:
                ...
                logger.info(
                    f"  🔍 {missing['name']} → potential matches: {potential_matches}"
                )
    ...
    def analyze_series_mapping(self):
        ...
        logger.info(
            f"  📊 Local series: {len(local_series)}, Railway series: {len(railway_series)}"
        )
    ...
    def create_missing_entities_plan(self):
        ...
        missing_clubs = [
            d
            for d in self.mapping_report["clubs"]["details"]
            if d["status"] == "missing"
        ]
        missing_series = [
            d
            for d in self.mapping_report["series"]["details"]
            if d["status"] == "missing"
        ]
        ...
        if missing_clubs:
            plan["recommendations"].append(
                f"Create {len(missing_clubs)} missing clubs in Railway"
            )

        if missing_series:
            plan["recommendations"].append(
                f"Create {len(missing_series)} missing series in Railway"
            )
            plan["recommendations"].append(
                "Alternative: Filter out players from unmapped series"
            )
    ...
    def validate_mappings(self):
        ...
        if club_ids:
            railway_cursor.execute(
                "SELECT id FROM clubs WHERE id = ANY(%s)", (club_ids,)
            )
            valid_club_ids = {row[0] for row in railway_cursor.fetchall()}
            invalid_club_mappings = {
                k: v for k, v in self.club_mapping.items() if v not in valid_club_ids
            }
        ...
        if series_ids:
            railway_cursor.execute(
                "SELECT id FROM series WHERE id = ANY(%s)", (series_ids,)
            )
            valid_series_ids = {row[0] for row in railway_cursor.fetchall()}
            invalid_series_mappings = {
                k: v
                for k, v in self.series_mapping.items()
                if v is not None and v not in valid_series_ids
            }

            if invalid_series_mappings:
                logger.error(
                    f"❌ Invalid series mappings found: {invalid_series_mappings}"
                )
                return False
    ...
    def migrate_players_safe(self):
        ...
        for player_row in players:
            ...
            if original_series_id is not None:
                if original_series_id in self.series_mapping:
                    ...
                    if mapped_series_id is not None:
                        ...
                        skip_reasons.append(
                            f"null-mapped series_id {original_series_id}"
                        )
                else:
            ...
            if skip_reasons:
                ...
                logger.debug(
                    f"  ⚠️  Skipped {player_dict['first_name']} {player_dict['last_name']}: {', '.join(skip_reasons)}"
                )
                continue
            ...
            try:
                ...
                if self.migration_stats["players"]["migrated"] % 500 == 0:
                    logger.info(
                        f"    ✅ Migrated {self.migration_stats['players']['migrated']} players..."
                    )
        ...
        logger.info(
            f"  ✅ Players migration: {self.migration_stats['players']['migrated']} migrated, {self.migration_stats['players']['skipped']} skipped"
        )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"  • Series mappings: {len([v for v in series_mapping.values() if v is not None])} successful"
        )
        ...
        local_cursor.execute(
            "SELECT COUNT(DISTINCT club_id) FROM players WHERE club_id IS NOT NULL"
        )
        unique_club_refs = local_cursor.fetchone()[0]

        local_cursor.execute(
            "SELECT COUNT(DISTINCT series_id) FROM players WHERE series_id IS NOT NULL"
        )
        ...
        logger.info(
            f"  • Players reference {unique_club_refs} unique clubs, we can map {mapped_clubs}"
        )
        logger.info(
            f"  • Players reference {unique_series_refs} unique series, we can map {mapped_series}"
        )
    ...
    try:
        ...
        if success:
            ...
            for table, stats in migrator.migration_stats.items():
                logger.info(
                    f"  • {table}: {stats['migrated']} migrated, {stats['skipped']} skipped"
                )
        else:
