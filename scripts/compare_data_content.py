...
def compare_databases():
    ...
    for table in sorted(local_counts.keys()):
        ...
        if isinstance(local_count, str) or isinstance(railway_count, str):
            ...
            if table in ['leagues', 'clubs', 'players', 'series']:
                critical_differences.append(f"{table}: Local has {local_count}, Railway has 0")
            ...
            if abs(local_count - railway_count) > local_count * 0.1:  # >10% difference
                data_sync_needed = True
                critical_differences.append(f"{table}: Local has {local_count}, Railway has {railway_count}")
        
        print(f"{table:<25} {str(local_count):<15} {str(railway_count):<15} {status:<20}")
        ...
        for table in key_tables:
            if local_counts.get(table, 0) > 0:
                ...
                if local_rows and len(local_rows) > 0:
                    print(f"    First record: {dict(zip(local_cols[:3], local_rows[0][:3]))}")
                
                # Railway sample
                railway_cols, railway_rows = get_sample_data(railway_conn, table, 3)
                print(f"  RAILWAY: {len(railway_rows) if railway_rows else 0} sample records")
                if railway_rows and len(railway_rows) > 0:
                    print(f"    First record: {dict(zip(railway_cols[:3], railway_rows[0][:3]))}")
    ...
    if data_sync_needed or critical_differences:
        ...
        print(f"  3. Reverse sync: Migrate from Railway to local (if Railway has newer data)")
