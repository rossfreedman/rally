...
def simple_migration():
    ...
    with get_db() as local_conn:
        ...
        try:
            ...
            logger.info("\n📋 MIGRATING PLAYERS:")
            player_count = migrate_table_simple(
                local_conn, railway_conn, "players", exclude_columns=["id"]
            )
            railway_conn.commit()

            # Step 3: Migrate player_history (excluding id)
            logger.info("\n📋 MIGRATING PLAYER HISTORY:")
            history_count = migrate_table_simple(
                local_conn, railway_conn, "player_history", exclude_columns=["id"]
            )
            railway_conn.commit()
            ...
            local_cursor.execute(
                "SELECT user_id, player_id, is_primary, created_at FROM user_player_associations"
            )
            ...
            if local_associations:
                ...
                local_users = {
                    email: local_id for local_id, email in local_cursor.fetchall()
                }
                ...
                local_players = {
                    tc_id: local_id for local_id, tc_id in local_cursor.fetchall()
                }
                ...
                for (
                    ...
                    local_cursor.execute(
                        "SELECT email FROM users WHERE id = %s", (local_user_id,)
                    )
                    ...
                    local_cursor.execute(
                        "SELECT tenniscores_player_id FROM players WHERE id = %s",
                        (local_player_id,),
                    )
            ...
            logger.info(
                f"  • Total new records: {player_count + history_count + association_count}"
            )
