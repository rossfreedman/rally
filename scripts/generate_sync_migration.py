    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "--autogenerate",
                "--message",
                f"sync_railway_to_local_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            ],
            capture_output=True,
            text=True,
        )
...
def check_pending_migrations():
    ...
    try:
        ...
        os.environ["SYNC_RAILWAY"] = "true"
        result = subprocess.run(
            [sys.executable, "-m", "alembic", "heads"], capture_output=True, text=True
        )
...
def main():
    ...
    print("\n" + "=" * 60)
    response = (
        input("🤔 Do you want to generate a sync migration? (y/N): ").strip().lower()
    )
