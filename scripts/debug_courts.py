...
def debug_court_assignments():
    ...
    for date_key in ross_dates:
        if date_key in matches_by_group:
            ...
            day_matches_sorted = sorted(day_matches, key=lambda m: (m.get('home_team', ''), m.get('away_team', '')), reverse=True)
            ...
            for i, match in enumerate(day_matches_sorted):
                ...
                if is_ross_match or court_num <= 5:  # Show Ross matches and first 5 courts
                    print(f"  Court {court_num}: {match['home_team']} vs {match['away_team']}{marker}")
            
            print()
    
    print("\n=== ANALYSIS ===")
    print("Now using league-filtered matches (NSTF only) with reverse alphabetical sorting.")
