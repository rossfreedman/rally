...
class GoogleAddressSearcher:
    ...
    def _find_gmb_address(self, soup):
        ...
        try:
            ...
            knowledge_panel = soup.find(
                "div", {"class": re.compile(r"knowledge|panel|kp-")}
            )
            if knowledge_panel:
                # Look for address text
                address_elements = knowledge_panel.find_all(
                    text=re.compile(r"\d+.*(?:St|Ave|Rd|Dr|Ln|Blvd|Way|Ct|Pl).*\d{5}")
                )
                if address_elements:
                    return address_elements[0].strip()

            # Alternative selector for address
            address_spans = soup.find_all(
                "span",
                string=re.compile(r"\d+.*(?:St|Ave|Rd|Dr|Ln|Blvd|Way|Ct|Pl).*\d{5}"),
            )
            if address_spans:
