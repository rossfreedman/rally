...
if all_pti_data:
    ...
    for i, record in enumerate(all_pti_data[:10]):
        ...
        print(f'  {i+1}. Player: {name} (ID: {record["player_id"]}) | Date: {record["date"]} | PTI: {record["end_pti"]} | Series: {record["series"]}')
...
for player in similar_players:
    print(f'  ID: {player["id"]} | Name: {player["first_name"]} {player["last_name"]} | Tennis ID: {player["tenniscores_player_id"]}') 