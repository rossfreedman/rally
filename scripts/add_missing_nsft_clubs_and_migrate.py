    ...
    try:
        for club_info in missing_nsft_clubs:
            ...
            if not existing_club and club_name == 'Midtown':
                railway_cursor.execute("SELECT id FROM clubs WHERE name = %s", ('Midtown - Chicago',))
                existing_club = railway_cursor.fetchone()
                if existing_club:
                    logger.info(f"  🔗 Mapping {club_name} to existing 'Midtown - Chicago' (ID: {existing_club[0]})")
            
            if existing_club:
                railway_id = existing_club[0]
                logger.info(f"  ✅ {club_name} already exists in Railway (ID: {railway_id})")
                ...
                logger.info(f"  ✅ Added {club_name} to Railway: local_id {local_id} → railway_id {railway_id} ({expected_players} players)")
        
        railway_conn.commit()
        logger.info(f"  🎯 Successfully processed {len(missing_nsft_clubs)} clubs: {added_count} added, {existing_count} already existed")
    ...
    with get_db() as local_conn:
        ...
        for local_id, local_name in local_clubs:
            ...
            if name_key in railway_by_name:
                ...
                missing_count += 1
                logger.warning(f"  ❌ Still missing: {local_name} (local_id: {local_id})")
        
        railway_conn.close()
        
        logger.info(f"  📊 Complete mapping: {mapped_count} mapped, {missing_count} still missing")
...
def migrate_players_with_updated_mappings(club_mapping, series_mapping):
    ...
    with get_db() as local_conn:
        ...
        for player_row in players:
            ...
            try:
                ...
                if migration_stats['migrated'] % 500 == 0:
                    logger.info(f"    ✅ Migrated {migration_stats['migrated']} players...")
        ...
        logger.info(f"  ✅ Migration complete: {migration_stats['migrated']} migrated, {migration_stats['skipped']} skipped")
...
def verify_migration_results():
    ...
    with get_db() as local_conn:
        ...
        print(f"  • NSFT Coverage: {(railway_nsft_count / local_nsft_count * 100):.1f}%")
...
def main():
    ...
    try:
        ...
        migration_stats = migrate_players_with_updated_mappings(
            complete_club_mapping, 
            complete_series_mapping
        )
        ...
        print(f"  • Overall coverage increased from 82.0% to {results['coverage_percent']:.1f}%")
        print(f"  • Added {results['improvement']} additional players")
        print(f"  • NSFT coverage increased from 58.3% to {results['nsft_coverage']:.1f}%")
