...
class IntegrationTester:
    ...
    def log_test(
        self, test_name: str, status: str, details: str = "", data: Any = None
    ):
        """Log test result"""
        result = {
            "test": test_name,
        ...
        try:
            ...
            if len(series_standings) >= 5:
                self.log_test(
                    ...
                    {
                        "team_count": len(series_standings),
                        "top_team": series_standings[0]["team"],
                    },
                )
                ...
                teams_with_matches = [
                    t
                    for t in series_standings
                    if t["matches_won"] + t["matches_lost"] > 0
                ]
                if len(teams_with_matches) >= len(series_standings) * 0.8:
                    self.log_test(
                        "Teams have match data",
                        "PASS",
                        f"{len(teams_with_matches)}/{len(series_standings)} teams have played matches",
                    )
                else:
                    self.log_test(
                        "Teams have match data",
                        "FAIL",
                        f"Only {len(teams_with_matches)}/{len(series_standings)} teams have match data",
                    )
            ...
            self.log_test(
                "My-series page", "FAIL", f"Error testing series data: {str(e)}"
            )
            ...
            if test_player:
                ...
                if player_stats and (player_stats["wins"] + player_stats["losses"]) > 0:
                    win_rate = round(
                        (
                            player_stats["wins"]
                            / (player_stats["wins"] + player_stats["losses"])
                        )
                        * 100,
                        1,
                    )
                    ...
                    self.log_test(
                        "Player statistics calculation",
                        "FAIL",
                        "Cannot calculate player win-loss record",
                    )
                ...
                self.log_test(
                    "Analyze-me player data",
                    "FAIL",
                    "No players found with sufficient match history (need >= 5 matches)",
                )

        except Exception as e:
            self.log_test(
                "Analyze-me page", "FAIL", f"Error testing player analysis: {str(e)}"
            )
        ...
        try:
            ...
            if test_team:
                ...
                self.log_test(
                    "My-team basic data",
                    "FAIL",
                    "No teams found with sufficient match data",
                )
    ...
    def test_availability_system(self):
        ...
        try:
            ...
            if future_matches["count"] >= 10:
                ...
                self.log_test(
                    "Future match schedule", "FAIL", "No future matches found"
                )
            ...
            self.log_test(
                "Availability system",
                "FAIL",
                f"Error testing availability data: {str(e)}",
            )
    ...
    def test_player_search_functionality(self):
        ...
        try:
            ...
            completeness_rate = (
                (complete_players["count"] / searchable_players["count"] * 100)
                if searchable_players["count"] > 0
                else 0
            )
            ...
            self.log_test(
                "Player search", "FAIL", f"Error testing player search data: {str(e)}"
            )
        ...
        for league_id, requirements in major_leagues.items():
            try:
                ...
                if league_data:
                    ...
                    if (
                        team_count >= requirements["min_teams"]
                        and player_count >= requirements["min_players"]
                    ):
                        self.log_test(
                    ...
                    self.log_test(
                        f"League {league_id} data", "FAIL", "No data found for league"
                    )
                ...
                self.log_test(
                    f"League {league_id}",
                    "FAIL",
                    f"Error testing league data: {str(e)}",
                )
    ...
    def run_all_integration_tests(self):
        ...
        try:
            ...
            self.log_test(
                "Integration test suite", "FAIL", f"Test suite error: {str(e)}"
            )
