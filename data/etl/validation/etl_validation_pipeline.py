...
class ETLValidator:
    ...
    def validate_table_completeness(self):
        ...
        for table_name, requirements in critical_tables.items():
            try:
                count_result = execute_query_one(f"SELECT COUNT(*) as count FROM {table_name}")
    ...
    def validate_data_relationships(self):
        ...
        if orphaned_players['count'] == 0:
            self.log_result("Player foreign key integrity", "PASS", "All players have valid league/club/series references")
        else:
            self.log_result("Player foreign key integrity", "FAIL", f"{orphaned_players['count']} players have invalid references")
        ...
        if orphaned_matches['count'] == 0:
            self.log_result("Match team references", "PASS", "All matches have valid team names")
        else:
            self.log_result("Match team references", "WARNING", f"{orphaned_matches['count']} matches missing team data")
        ...
        coverage = (teams_in_stats['count'] / teams_in_matches['count'] * 100) if teams_in_matches['count'] > 0 else 0
        
        if coverage >= 95:
            self.log_result("Series stats coverage", "PASS", f"{coverage:.1f}% of teams have statistics")
        elif coverage >= 85:
            self.log_result("Series stats coverage", "WARNING", f"Only {coverage:.1f}% coverage", ">= 95%", f"{coverage:.1f}%")
        else:
            self.log_result("Series stats coverage", "FAIL", f"Poor coverage: {coverage:.1f}%", ">= 95%", f"{coverage:.1f}%")
    ...
    def validate_statistical_accuracy(self):
        ...
        if not mismatched_points:
            self.log_result("Match win calculations", "PASS", "Series stats match actual match results")
        else:
            ...
            for team in mismatched_points[:3]:
                print(f"   {team['team']}: Stats={team['matches_won']}, Actual={team['actual_wins']}")
        ...
        if zero_point_winners['count'] == 0:
            self.log_result("Points calculation", "PASS", "Teams with wins have appropriate points")
        else:
            self.log_result("Points calculation", "WARNING", f"{zero_point_winners['count']} teams have wins but zero points")
    ...
    def validate_user_features(self):
        ...
        try:
            ...
            for league_id in major_leagues:
                ...
                if league_teams and league_teams['count'] > 10:
                    self.log_result(f"My-series data ({league_id})", "PASS", f"{league_teams['count']} teams available")
                elif league_teams and league_teams['count'] > 0:
                    self.log_result(f"My-series data ({league_id})", "WARNING", f"Only {league_teams['count']} teams", "> 10", league_teams['count'])
                else:
                    self.log_result(f"My-series data ({league_id})", "FAIL", "No series data available")
                    
        except Exception as e:
            self.log_result("My-series feature", "FAIL", f"Cannot test series data: {str(e)}")
        ...
        try:
            ...
            if recent_matches and recent_matches['count'] > 50:
                self.log_result("Availability system data", "PASS", f"{recent_matches['count']} recent matches for availability")
            elif recent_matches and recent_matches['count'] > 0:
                self.log_result("Availability system data", "WARNING", f"Only {recent_matches['count']} recent matches")
            else:
                self.log_result("Availability system data", "WARNING", "No recent matches found")
                
        except Exception as e:
            self.log_result("Availability feature", "FAIL", f"Cannot test availability data: {str(e)}")
    ...
    def validate_league_consistency(self):
        ...
        try:
            ...
            if not inconsistent_series:
                self.log_result("CNSWPL series consistency", "PASS", "All series have reasonable team counts")
            else:
                small_series = [f"{row['series']} ({row['team_count']} teams)" for row in inconsistent_series]
                self.log_result("CNSWPL series consistency", "WARNING", f"Small series found: {', '.join(small_series[:3])}")
                
        except Exception as e:
            self.log_result("League consistency check", "FAIL", f"Cannot validate: {str(e)}")
        ...
        try:
            ...
            if not duplicates:
                self.log_result("Team name uniqueness", "PASS", "No duplicate team names found")
            else:
                dup_teams = [f"{row['team']} in {row['league_id']}" for row in duplicates[:3]]
                self.log_result("Team name uniqueness", "WARNING", f"Duplicates found: {', '.join(dup_teams)}")
                
        except Exception as e:
            self.log_result("Duplicate check", "FAIL", f"Cannot check duplicates: {str(e)}")
