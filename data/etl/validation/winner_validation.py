...
def parse_score_to_determine_winner(
    score_str: str, league_id: str = None
) -> Optional[str]:
    """
...
def validate_match_winner(match_data: Dict) -> Dict:
    ...
    if calculated_winner is None:
        ...
        result["validation_note"] = (
            f"Improved unknown winner to {calculated_winner} based on score"
        )
    elif (
        recorded_winner in ["home", "away"]
        and calculated_winner
        and recorded_winner != calculated_winner
    ):
        # Mismatch - score analysis overrides
        result["needs_correction"] = True
        result["corrected_winner"] = calculated_winner
        result["validation_note"] = (
            f"Corrected {recorded_winner} to {calculated_winner} based on score analysis"
        )
    else:
        # Other cases
        result["validation_note"] = (
            f"Complex case: recorded={recorded_winner}, calculated={calculated_winner}"
        )
    ...
    for root, dirs, files in os.walk(leagues_dir):
        if "match_history.json" in files:
            ...
            if "error" not in file_result:
                ...
                print(
                    f"   {league_name}: {file_result['total_matches']} matches, "
...
def print_validation_summary(results: Dict):
    ...
    print(
        f"Consistent matches: {results['total_consistent']:,} ({results['total_consistent']/results['total_matches']*100:.1f}%)"
    )
    if results["total_corrected"] > 0:
        print(
            f"Corrected matches: {results['total_corrected']:,} ({results['total_corrected']/results['total_matches']*100:.1f}%)"
        )
    if results["total_problems"] > 0:
        print(
            f"Problematic matches: {results['total_problems']:,} ({results['total_problems']/results['total_matches']*100:.1f}%)"
        )
    ...
    for league, file_result in results["file_results"].items():
        ...
        if file_result["corrected_matches"] > 0:
            ...
            for correction in file_result["corrections"][:5]:  # Show first 5
                print(
                    f"   • {correction['home_team']} vs {correction['away_team']} ({correction['date']})"
                )
                print(f"     Score: {correction['scores']}")
                print(
                    f"     Change: {correction['original_winner']} → {correction['corrected_winner']}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate winner data in match history JSON files"
    )
    parser.add_argument(
        "--auto-fix", action="store_true", help="Automatically fix detected issues"
    )
    parser.add_argument(
        "--leagues-dir", help="Path to leagues directory (default: auto-detect)"
    )
    parser.add_argument(
        "--file", help="Validate specific JSON file instead of all files"
    )
