...
def run_master_etl_process():
    ...
    pre_check = run_script(
        "data/etl/validation/data_quality_monitor.py",
        "Pre-import data quality baseline",
    )
    ...
    validation_result = run_script(
        "data/etl/validation/etl_validation_pipeline.py", "ETL validation pipeline"
    )
    results.append(validation_result)

    # Phase 4: Integration Testing
    print("\n🧪 PHASE 4: INTEGRATION TESTING")
    integration_result = run_script(
        "data/etl/validation/integration_tests.py",
        "User-facing feature integration tests",
    )
    results.append(integration_result)

    # Phase 5: Final Data Quality Monitoring
    print("\n📊 PHASE 5: FINAL DATA QUALITY MONITORING")
    monitoring_result = run_script(
        "data/etl/validation/data_quality_monitor.py",
    ...
    critical_failures = [
        r
        for r in results
        if not r["success"] and "validation" in r["description"].lower()
    ]
    integration_failures = [
        r
        for r in results
        if not r["success"] and "integration" in r["description"].lower()
    ]
    ...
    if failed_phases > 0:
        ...
        for result in results:
            if not result["success"]:
                print(
                    f"   • {result['description']}: {result.get('error', 'Unknown error')}"
                )
