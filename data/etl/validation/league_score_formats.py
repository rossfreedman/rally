...
class ScoreFormatAnalyzer:
    def __init__(self):
        self.format_patterns = {
            ...
            "super_tiebreak_formal": re.compile(
                r"\d+-\d+, \d+-\d+, [01]-1 \[\d+-\d+\]"
            ),
    ...
    def print_league_analysis(self, stats: Dict):
        ...
        for format_type, count in sorted(
            stats["format_breakdown"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        if stats["issue_breakdown"]:
            ...
            for issue, count in sorted(
                stats["issue_breakdown"].items(), key=lambda x: x[1], reverse=True
            ):
                print(f"   {issue}: {count:,}")
        ...
        if stats["unusual_scores"]:
            ...
            for unusual in stats["unusual_scores"][:5]:
                print(
                    f"   • {unusual['home_team']} vs {unusual['away_team']} ({unusual['date']})"
                )
...
def main():
    ...
    super_tiebreak_leagues = [
        name for name, stats in all_stats.items() if stats["super_tiebreak_count"] > 0
    ]
    ...
    problematic_leagues = [
        (name, stats["invalid_matches"])
        for name, stats in all_stats.items()
        if stats["invalid_matches"] > 0
    ]
    if problematic_leagues:
        ...
        for name, invalid_count in problematic_leagues:
            ...
            print(
                f"   {name}: {invalid_count:,} invalid ({invalid_count/total*100:.1f}%)"
            )
