...
# Navigate up four levels: data/etl/scrapers -> data/etl -> data -> project_root
sys.path.append(
    os.path.dirname(
...
def discover_all_leagues_and_series(driver, config, max_exploration_pages=5):
    ...
    print(
        f"🔍 Discovering team IDs from {config['subdomain']} website using comprehensive discovery..."
    )
    ...
    def check_discovery_timeout():
        ...
        if elapsed > max_discovery_time:
            print(
                f"⏰ Discovery timeout reached ({elapsed:.1f}s > {max_discovery_time}s)"
            )
            return True
        return False
    ...
    try:
        ...
        if not check_discovery_timeout():
            print(
                f"🧭 Exploring navigation and menu links for additional team pages..."
            )
            ...
            for link in nav_links[:10]:  # Limit to first 10 links to prevent timeouts
                ...
                if any(
                    keyword in text
                    for keyword in ["standing", "team", "roster", "division", "league"]
                ):
                    if (
            ...
            for i, (text, href) in enumerate(
                potential_team_pages[:3]
            ):  # Limit to 3 to prevent timeouts
                if check_discovery_timeout():
                    break
                ...
                try:
                    ...
                    for link in page_team_links:
                        ...
                        if (
                            team_match
                            and team_name
                            and team_name not in discovered_teams
                        ):
                            # Skip BYE teams
                            if "BYE" in team_name.upper():
                                continue
            ...
            for i, pattern in enumerate(test_patterns):
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            f"⚠️ Driver error for pattern {pattern}: {str(driver_error)}"
                        )
                        continue
                    ...
                    try:
                        ...
                        if not page_soup:
                            print(
                                f"⚠️ BeautifulSoup parsing failed for pattern {pattern}"
                            )
                            continue

                    except Exception as parse_error:
                        print(
                            f"⚠️ Parse error for pattern {pattern}: {str(parse_error)}"
                        )
                        continue

                    # Wrap the team link extraction in its own try-catch
                    try:
                        page_team_links = page_soup.find_all(
                            "a", href=re.compile(r"team=")
                        )
                        ...
                        for link in page_team_links:
                            try:
                                ...
                                if (
                                    ...
                                    # Extract series and club info
                                    series_name = extract_series_name_from_team(
                                        team_name
                                    )
                                    if series_name:
                                        all_series.add(series_name)
                                ...
                                print(
                                    f"⚠️ Error processing link in pattern {pattern}: {str(link_error)}"
                                )
                                continue

                        if teams_found_in_pattern > 0:
                            print(
                                f"✅ Pattern {pattern} found {teams_found_in_pattern} new teams"
                            )
                        else:
                        ...
                        print(
                            f"⚠️ Team extraction error for pattern {pattern}: {str(extraction_error)}"
                        )
                        continue
        ...
        if not check_discovery_timeout() and len(discovered_teams) > 0:
            print(
                f"🔬 Deep-diving into {min(3, len(discovered_teams))} team pages for additional discovery..."
            )
        ...
        print(
            f"🏆 Teams: {len(discovered_teams)} (initial: {len(discovered_teams)}, new: 0)"
        )
        ...
        if not check_discovery_timeout():
            ...
            if is_platform_tennis:
                ...
                if found_numbers:
                    ...
                    for i in range(
                        min_found, min(max_found + 5, max_found + 10)
                    ):  # Limited range
            ...
            if missing_series:
                print(
                    f"🔍 Found {len(missing_series)} potentially missing series: {missing_series[:10]}{'...' if len(missing_series) > 10 else ''}"
                )
                ...
                for target_series in missing_series[:3]:  # Limit to 3 attempts
                    ...
                    try:
                        found_teams = find_missing_teams_comprehensive(
                            driver, config, discovered_teams, target_series
                        )
                        if found_teams:
    ...
    for attempt in range(max_retries):
        try:
                    ...
                    try:
                        ...
                        for line in lines:
                            ...
                            if line_clean == "W":
                                ...
                                losses += 1
                        print(
                            f"         ✅ Fallback W/L extraction: {wins}W, {losses}L"
                        )
            ...
            try:
                ...
                print(
                    f"         ✅ Extraction successful: PTI={result['PTI']}, W/L={result['Wins']}-{result['Losses']} ({result['Win %']})"
                )
                return result
...
def discover_league_teams(driver, config):
    ...
    try:
        ...
        for team_name, team_id in team_mapping.items():
            if team_name and (
                " - " in team_name  # APTA Chicago format: "Birchwood - 6"
                or "Chicago" in team_name  # APTA Chicago format
                or re.search(r"S\d+", team_name)  # NSTF format: "Birchwood S1"
                or "Series" in team_name  # NSTF format: "Series 1"
                or "Sunday" in team_name  # NSTF format: "Wilmette Sunday A"
                or re.search(
                    r"\s\d+[a-zA-Z]?$", team_name
                )  # CNSWPL format: "Birchwood 1", "Hinsdale PC 1a"
            ):
                # Skip BYE teams - they are placeholders with no actual players
                if "BYE" in team_name.upper():
        ...
        print(
            f"✅ Discovered {len(filtered_teams)} teams (filtered from {len(team_mapping)} total)"
        )
...
def scrape_team_players(team_id, driver, team_name, league_id, config):
    ...
    try:
        ...
        if not player_links:
            print(
                "   🔍 No player links found, searching for player names in tables..."
            )
        ...
        for i, link in enumerate(player_links):
            try:
                ...
                print(
                    f"    Player {i+1}: {first_name} {last_name} | {club_name} | ID: {player_id} | {'Captain' if is_captain else 'Co-Captain' if is_co_captain else 'Player'}"
                )
...
def scrape_league_players(league_subdomain, get_detailed_stats=False):
    ...
    try:
        ...
        print(
            "No filtering - comprehensive discovery and processing of all teams/series"
        )
        ...
        with StealthBrowserManager(headless=True) as driver:
            ...
            print(
                f"🚀 Starting {config['subdomain'].upper()} Player Scraper (Team-by-Team Mode)"
            )
            print(f"📊 Target Series: All Discovered Series")
            print(
                f"📈 Detailed Stats: {'Yes' if get_detailed_stats else 'No (faster)'}"
            )
            print("=" * 60)
            ...
            for team_num, (team_name, team_id) in enumerate(teams_to_scrape.items(), 1):
                ...
                team_players = scrape_team_players(
                    team_id, driver, team_name, league_id, config
                )
                ...
                if get_detailed_stats and team_players:
                    print(
                        f"   🔍 Extracting detailed stats for {len(team_players)} players..."
                    )
                    ...
                    if is_platform_tennis:
                        print(
                            f"   🏓 Platform Tennis League: Extracting PTI + Win/Loss stats"
                        )
                    else:
                        print(
                            f"   🎾 Tennis League: Extracting Win/Loss stats only (no PTI)"
                        )
                    ...
                    for player_idx, player in enumerate(team_players, 1):
                        ...
                        if player_url:
                            try:
                                ...
                                if is_platform_tennis:
                                    print(
                                        f"      🔍 [{player_idx}/{len(team_players)} - {player_progress:.1f}%] Getting PTI + stats for {player['First Name']} {player['Last Name']}"
                                    )
                                else:
                                    print(
                                        f"      🔍 [{player_idx}/{len(team_players)} - {player_progress:.1f}%] Getting W/L stats for {player['First Name']} {player['Last Name']}"
                                    )
                                ...
                                if stats["PTI"] != "N/A" and is_platform_tennis:
                                    player.update(stats)
                                    print(
                                        f"         ✅ Updated stats: PTI={stats['PTI']}, W/L: {stats['Wins']}-{stats['Losses']} ({stats['Win %']})"
                                    )
                                else:
                                    ...
                                    if is_platform_tennis:
                                        print(
                                            f"         ⚠️  No PTI found, but got W/L: {stats['Wins']}-{stats['Losses']} ({stats['Win %']})"
                                        )
                                    else:
                                        print(
                                            f"         ✅ Tennis stats: W/L: {stats['Wins']}-{stats['Losses']} ({stats['Win %']})"
                                        )

                            except Exception as e:
                                print(
                                    f"         ❌ Error getting stats for {player['First Name']} {player['Last Name']}: {e}"
                                )
                                continue
                        else:
                            print(
                                f"      ⚠️  No URL available for {player['First Name']} {player['Last Name']}"
                            )
                        ...
                        if is_platform_tennis:
                            time.sleep(
                                0.5
                            )  # Platform tennis needs more time for PTI extraction
                        else:
                ...
                avg_time_per_team = (
                    (team_start_time - scraping_start_time).total_seconds() / team_num
                    if scraping_start_time and team_num > 0
                    else 0
                )
                estimated_remaining = (
                    remaining_teams * avg_time_per_team if avg_time_per_team > 0 else 0
                )
                eta = (
                    team_end_time + timedelta(seconds=estimated_remaining)
                    if estimated_remaining > 0
                    else None
                )

                print(
                    f"✅ Team completed in {team_duration.total_seconds():.1f}s | Progress: {team_num}/{total_teams} teams, {len(all_players)} players"
                )
                if eta:
                    print(
            ...
            players_per_minute = (
                (len(all_players) / total_seconds * 60) if total_seconds > 0 else 0
            )
            ...
            print(
                f"⚡ Average time per team: {total_seconds/total_teams:.1f}s"
                if total_teams > 0
                else "⚡ Average time per team: N/A"
            )
            print(
                f"⚡ Average time per player: {total_seconds/len(all_players):.1f}s"
            ...
            for series, count in sorted(series_counts.items()):
                percentage = (
                    (count / len(all_players) * 100) if len(all_players) > 0 else 0
                )
...
class PlayerScraper:
    ...
    def extract_player_stats(self, player_url, retries=3):
        ...
        for attempt in range(retries):
            try:
                print(
                    f"         🔍 Extracting stats from: {player_url} (Attempt {attempt + 1})"
                )
                ...
                if self.is_platform_tennis:
                    if pti_value:
                        print(
                            f"         ✅ Found PTI: {pti_value}, W/L: {wins}-{losses} ({win_percentage})"
                        )
                        self.successful_pti_extractions += 1
                    else:
                        print(
                            f"         ⚠️ No PTI found, but got W/L: {wins}-{losses} ({win_percentage})"
                        )
                        self.failed_pti_extractions.append(player_url)
                else:
                    print(
                        f"         ✅ Tennis league - skipping PTI, got W/L: {wins}-{losses} ({win_percentage})"
                    )
                ...
                if re.match(r"^\d+\.?\d*$", line):
                    ...
                    if 10.0 <= pti <= 100.0:
                        ...
                        if any(
                            ...
                            for keyword in ["player", "start", "end", "rating", "match"]
                        ):
                            return str(pti)
    ...
    def _extract_win_loss_stats(self, soup):
        ...
        try:
            ...
            for line in lines:
                ...
                season_pattern = (
                    r"Chicago:\s*\d+\s*\((\d+)\s*Wins?,\s*(\d+)\s*Losses?\)"
                )
                ...
                if match:
                    ...
                    print(
                        f"         Found season: {line} -> +{season_wins}W, +{season_losses}L"
                    )
            ...
            print(
                "         No season summaries found, counting individual match results..."
            )
...
def find_missing_teams_comprehensive(driver, config, known_teams, target_series=None):
            ...
            for link in team_links:
                ...
                if team_match and team_name and team_name not in known_teams:
                    ...
                    if target_series and series_name == target_series:
                        print(
                            f"      🎉 FOUND TARGET: {team_name} -> {team_id} (series: {series_name})"
                        )
                    elif series_name:
                        print(
                            f"      ✅ Found: {team_name} -> {team_id} (series: {series_name})"
                        )
        ...
        if series_match:
            ...
            for i, params in enumerate(param_combinations):
                try:
                    ...
                    print(
                        f"   🧪 Testing specific params {i+1}/{len(param_combinations)}: {param_string}"
                    )
                    driver.get(test_url)
                    time.sleep(2)
                    ...
                    for link in team_links:
                        ...
                        if team_match and team_name and team_name not in known_teams:
                            ...
                            if series_name == target_series:
                                new_teams[team_name] = team_id
                                print(
                                    f"      🎯 FOUND TARGET SERIES TEAM: {team_name} -> {team_id}"
                                )
    ...
    if is_platform_tennis:
        ...
        if numeric_ids:
            ...
            for test_id in test_range:
                if str(test_id) not in known_team_ids:
                    try:
                        test_url = (
                            f"{base_url}/?mod={config['team_page_mod']}&team={test_id}"
                        )
                        driver.get(test_url)
                        time.sleep(1)  # Reduced delay
                            ...
                            for header in headers:
                                ...
                                if header_text and len(header_text) > 3:
                                    if "BYE" not in header_text.upper():
                                        ...
                                        print(
                                            f"      🔢 Found by ID: {header_text} -> {test_id}"
                                        )
                                        break
        ...
        print(
            f"   ⏭️ Skipping sequential ID discovery for tennis league (prevents timeouts)"
        )
