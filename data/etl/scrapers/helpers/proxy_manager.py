...
class EnhancedProxyRotator:
    ...
    def _should_rotate(self) -> bool:
        ...
        total_proxies = len(self.ports)
        if available_proxies < (total_proxies * 0.5):  # Less than 50% available - more aggressive recovery
        ...
        # Attempt proxy recovery every 25 requests if we have dead proxies
        if self.request_count % 25 == 0 and self.request_count > 0 and self.dead_proxies:
            logger.info(f"🔄 Periodic recovery attempt - {len(self.dead_proxies)} dead proxies")
            self._attempt_proxy_recovery()
    ...
    def _attempt_proxy_recovery(self):
        ...
        # Limit recovery attempts to avoid slowing down the scraper
        max_recovery_attempts = min(10, len(self.dead_proxies))
        dead_proxies_to_test = list(self.dead_proxies)[:max_recovery_attempts]
        
        logger.info(f"🔄 Attempting to recover {len(dead_proxies_to_test)} dead proxies (of {len(self.dead_proxies)} total)...")
        recovered_count = 0
        
        for port in dead_proxies_to_test:
            try:
                # Test the dead proxy
                ...
                response = requests.get(self.test_url, proxies=proxies, timeout=8, headers=self._get_headers())
                if response.status_code == 200 and len(response.text) > 50:
                    # Proxy is working again - reset its status
                    self.dead_proxies.discard(port)
                    proxy_info.is_healthy = True
                    proxy_info.consecutive_failures = 0
                    proxy_info.status = ProxyStatus.HEALTHY
                    proxy_info.success_rate = max(proxy_info.success_rate, 60)  # Give it a decent starting success rate
                    recovered_count += 1
        ...
        if recovered_count > 0:
            ...
            logger.info(f"❌ No proxies could be recovered from test batch")
    ...
    def _restart_proxy_session(self):
        ...
        # Clear capped proxies (always safe to reset usage caps)
        self.capped_proxies.clear()
        self.proxy_usage.clear()
        
        # Only clear dead proxies if we have very few available proxies
        available_count = len([p for p in self.proxies.values() if p.is_healthy and p not in self.dead_proxies])
        if available_count < len(self.ports) * 0.1:  # Less than 10% available
            logger.info("🔄 Clearing dead proxies due to critical shortage")
            self.dead_proxies.clear()
        else:
            logger.info(f"🔄 Keeping {len(self.dead_proxies)} dead proxies (have {available_count} available)")
        ...
        # Reset proxy health for non-dead proxies only
        for port, info in self.proxies.items():
            if port not in self.dead_proxies:
                info.is_healthy = True
                info.consecutive_failures = max(0, info.consecutive_failures - 2)  # Reduce failures instead of clearing
