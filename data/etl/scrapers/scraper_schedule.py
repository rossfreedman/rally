...
def scrape_tennis_schedule(league_subdomain):
    ...
    try:
        ...
        for series_idx, element in enumerate(series_elements, 1):
            ...
            try:
                ...
                if series_link and series_link.text:
                    ...
                    if not full_url:
                        print(f"⚠️  Skipping series {formatted_series} - No valid URL found")
                    ...
                    if not schedule_link:
                        print(f"   ⚠️  No schedule link found for series {formatted_series}")
                        skipped_series_count += 1
                        continue
                    ...
                    print(f"   📋 Found {len(schedule_entries)} potential schedule entries")
                    ...
                    for entry in schedule_entries:
                        ...
                        if all([date, time, home, away]):  # Only require core fields
                            ...
                            if 'BYE' in raw_home.upper() or 'BYE' in raw_away.upper():
                                print(f"       Skipping BYE schedule entry: {raw_home} vs {raw_away}")
                                continue
                    ...
                    print(f"   ✅ Added {entries_added} schedule entries for {formatted_series}")
                    ...
                    print(f"✅ Series completed in {series_duration.total_seconds():.1f}s | Progress: {series_idx}/{len(series_elements)} series")
                    if eta:
        ...
        entries_per_minute = (len(all_schedule_data) / total_seconds * 60) if total_seconds > 0 else 0
        ...
        for series, count in sorted(series_counts.items()):
            percentage = (count / len(all_schedule_data) * 100) if len(all_schedule_data) > 0 else 0
