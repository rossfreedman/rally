...
def backup_user_associations():
    ...
    try:
        with get_db() as conn:
            ...
            print(f"✅ Backed up {len(users)} users and {len(associations)} associations to {backup_file}")
            return backup_file
        ...
        with get_db() as conn:
            ...
            for user in users:
                ...
                """, (
                    ...
                    user['first_name'], user['last_name'], user['club_automation_password'],
                    user['created_at'], user['last_login'], user['is_admin'],
                    user['league_id'], user['tenniscores_player_id']
                ))
                restored_users += 1
            ...
            for assoc in associations:
                ...
                if player_result:
                    ...
                    """, (
                        assoc['user_id'], new_player_id, 
                        assoc['is_primary'], assoc['created_at']
                    ))
                    restored_associations += 1
                else:
                    print(f"⚠️  Could not find player {assoc['tenniscores_player_id']} in league {assoc['league_id']}")
