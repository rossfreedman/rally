...
def auto_fix_broken_league_contexts(cursor, logger=None) -> int:
    """
    Find and fix users with broken league contexts (pointing to non-existent league IDs)
    
    Returns:
        int: Number of broken contexts fixed
    """
    def log(message):
        if logger:
            logger.log(message)
    ...
    try:
        ...
        if not broken_users:
            log("✅ No broken league contexts found")
            return 0
        ...
        log(f"✅ Fixed {fixed_count} broken league contexts")
        return fixed_count
