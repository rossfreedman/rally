    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if version:
                ...
                result = subprocess.run(["alembic", "stamp", local_version], env=env, capture_output=True, text=True)
                if result.returncode == 0:
    ...
    try:
        ...
        for table, local_count in local_counts.items():
            ...
            if local_count == railway_count:
                ...
                logger.warning(f"⚠️  {table}: Local={local_count}, Railway={railway_count}")
        
        success_rate = (matches / total) * 100 if total > 0 else 0
        logger.info(f"Clone success rate: {matches}/{total} tables ({success_rate:.1f}%)")
