...
class DatabaseMirror:
    ...
    def sync_alembic_migration_state(self, railway_version):
        ...
        if not railway_version:
            logger.warning(
                "⚠️  No Railway migration version found, skipping Alembic sync"
            )
            return True
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"Current local migration state: {verify_result.stdout.strip()}"
                )
                return True
            else:
    ...
    def verify_mirror_success(self):
        ...
        try:
            ...
            if all_match:
                logger.info(
                    "✅ Mirror verification successful - all table counts match!"
                )
            else:
                logger.error(
                    "❌ Mirror verification failed - table count mismatches detected"
                )
    ...
    def mirror_database(self):
        ...
        if not self.sync_alembic_migration_state(railway_version):
            logger.error(
                "❌ Failed to sync Alembic state. Manual intervention may be needed."
            )
