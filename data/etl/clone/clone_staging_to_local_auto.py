#!/usr/bin/env python3
"""
Clone Railway Staging Database to Local using Alembic (Auto Mode)

This script creates an exact clone of the Railway staging database (schema + data) on Local
and ensures Alembic migration state is synchronized - WITHOUT user confirmation prompts.

Usage:
    python data/etl/clone/clone_railway_staging_to_local_auto.py
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(logs_dir, "database_clone_staging_reverse.log")),
...
class DatabaseCloner:
    ...
    def get_railway_migration_version(self):
        """Get the current Alembic migration version from Railway staging database"""
        logger.info("Checking Railway staging database migration version...")
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if cursor.fetchone()[0]:
                    ...
                    if version:
                        logger.info(f"Railway staging database migration version: {version[0]}")
                        return version[0]
                    else:
                        logger.warning(
                            "Railway staging database has alembic_version table but no version"
                        )
                        return None
                else:
                    logger.warning("Railway staging database has no alembic_version table")
                    return None

            conn.close()

        except Exception as e:
            logger.error(f"❌ Failed to get Railway staging migration version: {e}")
            return None

    def dump_railway_database(self):
        """Create a complete dump of the Railway staging database"""
        dump_file = f"railway_staging_dump_for_local_{self.backup_timestamp}.sql"
        logger.info(f"Creating Railway staging database dump: {dump_file}")
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"✅ Railway staging database dumped successfully to {dump_file}")
                ...
                logger.error(f"❌ Railway staging dump failed: {result.stderr}")
                return None

        except Exception as e:
            logger.error(f"❌ Railway staging dump error: {e}")
            return None
    ...
    def restore_railway_dump_to_local(self, dump_file):
        """Restore Railway staging dump to local database"""
        logger.info(f"Restoring Railway staging dump {dump_file} to local database...")
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info("✅ Railway staging dump restored to local database successfully")
                if result.stderr:
    ...
    def sync_local_alembic_state(self, railway_version):
        """Sync local Alembic state with Railway staging"""
        if not railway_version:
            logger.warning("⚠️  No Railway staging migration version found, skipping Alembic sync")
            return True
        ...
        try:
            ...
            if "SYNC_RAILWAY_STAGING" in env:
                del env["SYNC_RAILWAY_STAGING"]

            # Set the local alembic version to match Railway staging
        ...
        try:
            ...
            for table in railway_counts:
                ...
                if difference == 0:
                    ...
                    logger.warning(
                        f"⚠️  {table}: Railway Staging={railway_count}, Local={local_count} (small difference: {difference} rows)"
                    )
                    total_differences += difference
                else:
                    logger.error(
                        f"❌ {table}: Railway Staging={railway_count}, Local={local_count} (LARGE MISMATCH: {difference} rows)"
                    )
                    all_match = False

            # Check for tables that exist in local but not Railway staging
            for table in local_counts:
                if table not in railway_counts:
                    logger.warning(f"⚠️  {table}: Exists in local but not in Railway staging")
    ...
    def clone_database(self):
        ...
        logger.info("🚀 Starting Railway staging to local database clone process...")
        ...
        # Step 2: Get Railway staging migration version
        ...
        # Step 4: Dump Railway staging database
        dump_file = self.dump_railway_database()
        if not dump_file:
            logger.error("❌ Failed to dump Railway staging database. Aborting.")
            return False
        ...
        # Step 6: Restore Railway staging dump to local
        if not self.restore_railway_dump_to_local(dump_file):
            logger.error("❌ Failed to restore Railway staging dump to local. Aborting.")
            return False
        ...
        logger.info("🎉 Railway staging to local database clone completed successfully!")
        if backup_file:
        ...
        logger.info("🔄 Local database now matches Railway staging exactly (schema + data)")
...
def main():
    ...
    print("Railway Staging to Local Database Clone Tool (Auto Mode)")
    print("=" * 60)
    print("⚠️  This will REPLACE local database with Railway STAGING data automatically!")
    ...
    if success:
        ...
        print("Local database now exactly matches your Railway staging database.")
    else:
