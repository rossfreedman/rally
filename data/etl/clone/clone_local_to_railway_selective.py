...
class SelectiveDatabaseCloner:
    def __init__(self, tables_to_clone=None):
        ...
        if not self.tables_to_clone:
            raise ValueError("No tables specified for cloning. Please configure TABLES_TO_CLONE.")
    ...
    def verify_tables_exist(self):
        ...
        if missing_local:
            logger.error(f"❌ Tables missing from local database: {', '.join(missing_local)}")
        if missing_railway:
            logger.error(f"❌ Tables missing from Railway database: {', '.join(missing_railway)}")
    ...
    def backup_railway_tables(self):
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"✅ Railway tables backed up successfully to {backup_file}")
                return backup_file
            else:
    ...
    def verify_clone_success(self):
        ...
        try:
            ...
            if all_match:
                logger.info("✅ Selective clone verification successful - all specified table counts match!")
            else:
                logger.error("❌ Selective clone verification failed - table count mismatches detected")
    ...
    def clone_tables(self):
        ...
        if not backup_file:
            logger.warning("⚠️  Failed to backup Railway tables. Proceeding without backup.")
            backup_file = None
        ...
        logger.info(f"🔄 Railway tables now match local: {', '.join(self.tables_to_clone)}")
