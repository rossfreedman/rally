...
class DatabaseCloner:
    ...
    def backup_railway_database(self):
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(
                    f"✅ Railway database backed up successfully to {backup_file}"
                )
                return backup_file
            else:
    ...
    def sync_railway_alembic_state(self, local_version):
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(
                    f"✅ Railway Alembic migration state synced to {local_version}"
                )
                return True
            else:
    ...
    def clone_database(self):
        ...
        if not backup_file:
            logger.warning(
                "⚠️  Failed to backup Railway database due to version mismatch. Proceeding without backup."
            )
        ...
        if not self.sync_railway_alembic_state(local_version):
            logger.error(
                "❌ Failed to sync Railway Alembic state. Manual intervention may be needed."
            )
        ...
        if backup_file:
            ...
            logger.info(
                "📁 No Railway backup created due to version compatibility issues"
            )
