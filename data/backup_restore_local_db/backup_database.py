...
def create_backup(
    backup_format="sql", schema_only=False, data_only=False, max_backups=10
):
    ...
    try:
        ...
        extensions = {"sql": ".sql", "custom": ".dump", "tar": ".tar", "directory": ""}
        ...
        if backup_format == "directory":
            backup_size = sum(
                f.stat().st_size for f in Path(backup_path).rglob("*") if f.is_file()
            )
        else:
        ...
        metadata_path = (
            ...
            if backup_format != "directory"
            else os.path.join(backup_path, "backup_metadata.json")
        )
...
def list_backups():
    ...
    try:
        ...
        for item in os.listdir(backup_dir):
            ...
            if item.startswith("rally_db_backup_") and (
                ...
                metadata_path = (
                    ...
                    else os.path.join(item_path, "backup_metadata.json")
                )
                metadata = {}
        ...
        logger.info(
            f"Total backup space: {total_size_mb:.2f} MB ({total_size_gb:.2f} GB)"
        )
...
def main():
    ...
    parser.add_argument(
        "--format",
        choices=["sql", "custom", "tar", "directory"],
        default="sql",
    ...
    parser.add_argument(
        "--data-only", action="store_true", help="Backup data only, no schema"
    )
    parser.add_argument(
        "--max-backups",
        type=int,
