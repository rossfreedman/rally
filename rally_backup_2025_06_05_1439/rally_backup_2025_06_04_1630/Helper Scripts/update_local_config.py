    ...
    for file_path in host_port_files:
        if os.path.exists(file_path):
            ...
            if "trolley.proxy.rlwy.net" in content and "34555" in content:
                ...
                new_content = content.replace("trolley.proxy.rlwy.net", "ballast.proxy.rlwy.net")
                new_content = new_content.replace("34555", "40911")
                new_content = new_content.replace("OoxuYNiTfyRqbqyoFTNTUHRGjtjHVscf", "HKJnPmxKZmKiIglQhQPSmfcAjTgBsSIq")
