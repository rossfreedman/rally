...
class CNSWPLRosterScraper:
    ...
    def get_career_stats_from_individual_page(self, player_url: str, max_retries: int = 2) -> Dict[str, any]:
        """
        Get career wins and losses from individual player page - OPTIMIZED for speed.
        Uses proxy-first approach with smart browser fallback.
        ...
        for attempt in range(max_retries):
            try:
                ...
                # OPTIMIZATION 1: Skip proxy for career stats (proxy never has career stats)
                # Career stats require browser automation for Chronological checkbox interaction
                print(f"   🌐 Using optimized browser automation for career stats...")
                
                # Use existing stealth browser
                ...
                # OPTIMIZATION 3: Smart detection - only click if we don't have career stats
                if not self._has_career_stats_in_html(html_content):
                    print(f"   🔄 No career stats detected, enabling chronological view...")
                    
                    try:
                        driver = getattr(browser, 'current_driver', None)
                        if driver:
                            # OPTIMIZATION 4: Reduced delays
                            time.sleep(1)  # Reduced from 2
                            
                            # Find Chronological checkbox (optimized approach)
                            chronological_checkbox = driver.find_elements("xpath", "//input[@type='checkbox'][contains(@name, 'chron')]")
                            if not chronological_checkbox:
                                chronological_checkbox = driver.find_elements("xpath", "//input[@type='checkbox']")
                            ...
                            if chronological_checkbox:
                                ...
                                # Click Expand All Matches (optimized)
                                expand_button = driver.find_elements("xpath", "//input[@type='button'][@value='Expand All Matches']")
                ...
                # OPTIMIZATION 5: Extract career stats
    ...
    def _extract_career_stats_from_html(self, html_content: str) -> Dict[str, str]:
        """
        Extract career stats from HTML using optimized word boundary matching.
        """
        wins = 0
        losses = 0
