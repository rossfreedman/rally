...
def find_topic_data(training_data, topic):
    ...
    for key in training_data.keys():
        ...
        if (
            score > best_score and score >= len(topic_words) // 2
...
def get_training_data_by_topic():
    ...
    try:
        ...
        if not topic:
            return (
                jsonify({"error": "Topic parameter is required and cannot be empty"}),
                400,
            )
        ...
        if not topic_data:
            ...
            available_topics = list(training_data.keys())[
                :10
            ]  # First 10 topics as sample
        ...
        response_data = {"topic": topic_key, "data": topic_data}

        # Log successful retrieval
        logger.info(
            f"Training data successfully retrieved for topic: '{topic_key}' (requested: '{topic}')"
        )
...
def get_training_topics():
    ...
    try:
        ...
        return jsonify({"topics": topics, "total_count": len(topics)}), 200
