        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "available_topics_sample" in data:
                    print(
                        f"   💡 Suggestion: {data.get('suggestion', 'Try different keywords')}"
                    )
            else:
...
def test_local_module():
    ...
    try:
        from api.training_data import find_topic_data, load_training_data
...
if __name__ == "__main__":
    ...
    if user_input == "y":
        ...
        print(
            "Skipping API endpoint tests. Start the server and run this script again."
        )
