    ...
    try:
        ...
        if team_id:
            ...
            try:
                ...
                print(f"Warning: Error loading matches data for team filtering: {str(e)}")
                team_id = None  # Continue without team filtering
        
        # Filter by team if specified and add stats from JSON (temporary)
        final_players = []
        if team_id and team_players:
            # Filter players to only those on the team
            for player in all_players:
                if player['name'] in team_players:
                    # Add placeholder stats (these should come from database in future)
                    player.update({
                        'rating': '0',
                        'wins': '0', 
                        'losses': '0',
            ...
            # Return all players with placeholder stats
            for player in all_players:
                player.update({
                    'rating': '0',
                    'wins': '0',
        ...
        print(f"Found {len(final_players)} players in {series}{' for team ' + team_id if team_id else ''} and club {user_club}")
        print("=== END DEBUG ===\n")
