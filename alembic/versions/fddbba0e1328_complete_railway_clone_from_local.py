...
def get_all_local_data():
    ...
    data = {
        'leagues': [], 'clubs': [], 'series': [], 'users': [], 'players': [],
        'player_history': [], 'user_player_associations': [], 'match_scores': [],
        'schedule': [], 'series_stats': [], 'player_availability': [],
            ...
            try:
                ...
                for row in local_cursor.fetchall():
                    data['user_activity_logs'].append({
                        'id': row[0], 'user_id': row[1], 'action': row[2], 'details': row[3], 'timestamp': row[4]
                    })
            ...
            try:
                ...
                for row in local_cursor.fetchall():
                    data['user_instructions'].append({
                        'id': row[0], 'user_id': row[1], 'instruction_text': row[2], 'created_at': row[3], 'status': row[4]
                    })
...
def clear_railway_data(connection):
    ...
    tables_to_clear = [
        'user_activity_logs', 'user_instructions', 'player_availability', 
        'series_stats', 'schedule', 'match_scores', 'user_player_associations', 
        'player_history', 'players', 'users', 'series', 'clubs', 'leagues'
    ]
...
def fix_sequences(connection):
    ...
    tables_with_sequences = [
        ...
        ('users', 'id'), ('players', 'id'), ('match_scores', 'id'), 
        ('schedule', 'id'), ('series_stats', 'id'), ('player_availability', 'id'),
        ('user_activity_logs', 'id'), ('user_instructions', 'id')
    ]
...
def upgrade():
    ...
    if local_data['user_activity_logs']:
        import_table_data(connection, 'user_activity_logs', local_data['user_activity_logs'],
                         ['id', 'user_id', 'action', 'details', 'timestamp'])
    
    if local_data['user_instructions']:
        import_table_data(connection, 'user_instructions', local_data['user_instructions'],
