...
def get_local_data():
    ...
    try:
        with get_db() as local_conn:
            ...
            railway_cursor = railway_conn.execute(sa.text("SELECT tenniscores_player_id, id FROM players WHERE tenniscores_player_id IS NOT NULL"))
            ...
            print(f"   ✅ Created mappings for {len(local_data['player_mappings']):,} players")
            ...
            if local_data['player_mappings']:
                ...
                print(f"   ✅ Found {len(local_data['player_history']):,} player_history records to import")
            ...
            print(f"   ✅ Found {len(local_data['user_player_associations']):,} user_player_associations to import")
...
def upgrade():
    ...
    current_ph = connection.execute(sa.text("SELECT COUNT(*) FROM player_history")).scalar()
    current_upa = connection.execute(sa.text("SELECT COUNT(*) FROM user_player_associations")).scalar()
    ...
    if local_data['player_history']:
        print(f"📊 Importing {len(local_data['player_history']):,} player_history records...")
        
        # Use bulk insert for better performance
        player_history_table = sa.table('player_history',
    ...
    if local_data['user_player_associations']:
        print(f"👥 Importing {len(local_data['user_player_associations']):,} user_player_associations...")
        ...
        if association_data:
            user_player_assoc_table = sa.table('user_player_associations',
    ...
    print(f"   user_player_associations: {final_upa:,} records (+{final_upa - current_upa:,})")
