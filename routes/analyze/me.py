...
def get_player_analysis(user):
    ...
    try:
        ...
        for _, match in recent_matches.iterrows():
            is_team1 = (
                match["player1"] == user["name"] or match["player2"] == user["name"]
            )
            won = (is_team1 and match["team1_won"]) or (
                not is_team1 and match["team2_won"]
            )
            recent_form.append(
                {
                    "date": (
        ...
        return {
            ...
            "win_rate": (
                round((wins / total_matches) * 100, 1) if total_matches > 0 else 0
            ),
            "preferred_position": (
                "Forehand"
                if position_counts["forehand"] > position_counts["backhand"]
                else "Backhand"
            ),
        ...
        try:
            ...
            if "error" in analysis:
                return jsonify(analysis), (
                    404 if analysis["error"] == "No match data found" else 500
                )
