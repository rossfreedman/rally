    ...
    try:
        ...
        for _, match in matches.iterrows():
            ...
            for player in players:
                ...
                player_matches[player].append({"date": match["date"], "won": won})
        ...
        return sorted(
            streaks,
            key=lambda x: (x["max_win_streak"], -x["max_loss_streak"]),
            reverse=True,
        )
...
def get_club_analysis(user):
    ...
    try:
        ...
        if matches_df.empty:
            return {"error": "No match data found", "matches_played": 0, "teams": 0}
...
def init_routes(app):
    ...
    def my_club():
        try:
            ...
            return jsonify({"user": user, "analysis": analysis})
