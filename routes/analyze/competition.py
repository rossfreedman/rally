...
def get_player_court_stats(player_name):
    ...
    try:
        with get_db() as conn:
            ...
            """,
                conn,
                params=[player_name] * 4,
            )

            if matches_df.empty:
                return {"error": "No match data found", "matches_played": 0}
            ...
            wins = len(
                matches_df[
                    ...
                    | (
                        (matches_df["player3"] == player_name)
                        & (matches_df["team2_won"] == 1)
                    )
                    | (
                        (matches_df["player4"] == player_name)
                        & (matches_df["team2_won"] == 1)
                    )
                ]
            )
            ...
            for court in matches_df["court"].unique():
                ...
                court_wins = len(
                    court_matches[
                        (
                            (court_matches["player1"] == player_name)
                            & (court_matches["team1_won"] == 1)
                        )
                        | (
                            (court_matches["player2"] == player_name)
                            & (court_matches["team1_won"] == 1)
                        )
                        | (
                            (court_matches["player3"] == player_name)
                            & (court_matches["team2_won"] == 1)
                        )
                        | (
                            (court_matches["player4"] == player_name)
                            & (court_matches["team2_won"] == 1)
                        )
                    ]
                )
    ...
    try:
        with get_db() as conn:
            ...
            wins = len(
                matches_df[
                    (
                        (matches_df["team1_id"] == team_id)
                        & (matches_df["team1_won"] == 1)
                    )
                    | (
                        (matches_df["team2_id"] == team_id)
                        & (matches_df["team2_won"] == 1)
                    )
                ]
            )
            ...
            for _, match in matches_df.iterrows():
                ...
                opponent_name = (
                    match["team2_name"]
                    if match["team1_id"] == team_id
                    else match["team1_name"]
                )
                won = (match["team1_id"] == team_id and match["team1_won"]) or (
                    match["team2_id"] == team_id and match["team2_won"]
                )
...
def init_routes(app):
        ...
        try:
            ...
            if "error" in stats:
                return jsonify(stats), (
                    404 if stats["error"] == "No match data found" else 500
                )
    ...
    def research_team():
        try:
            ...
            if "error" in stats:
                return jsonify(stats), (
                    404 if stats["error"] == "Team not found" else 500
                )
