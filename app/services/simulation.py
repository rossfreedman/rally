...
class AdvancedMatchupSimulator:
    ...
    def simulate_matchup(self, team_a_players: List[int], team_b_players: List[int], 
                        user_league_id: Optional[str] = None) -> Dict:
        """
        ...
        """
        try:
            print(f"[DEBUG] Starting simulation: Team A {team_a_players} vs Team B {team_b_players}")
            ...
            print(f"[DEBUG] Composite scores: Team A = {team_a_score:.3f}, Team B = {team_b_score:.3f}")
            
            # Calculate win probability
            win_probability = self._calculate_win_probability(team_a_score, team_b_score)
            
            # Generate analysis
            advantages = self._analyze_advantages(team_a_metrics, team_b_metrics, team_a_data, team_b_data)
            confidence = self._determine_confidence(team_a_metrics, team_b_metrics, win_probability)
            ...
            return {
                ...
                "metric_breakdown": self._get_metric_breakdown(team_a_metrics, team_b_metrics)
            }
    ...
    def _get_team_data(self, player_ids: List[int], league_id: Optional[int] = None) -> Dict:
        ...
        try:
            ...
            for player_id in player_ids:
                ...
                players.append({
                    ...
                    "pti": float(player_data['pti']) if player_data['pti'] else 25.0,  # Default PTI
                    "wins": player_data['wins'] or 0,
                    "losses": player_data['losses'] or 0,
                    "win_percentage": float(player_data['win_percentage']) if player_data['win_percentage'] else 0.0
                })
    ...
    def _calculate_real_metrics(self, player_ids: List[int], league_id: Optional[int] = None, opponent_player_ids: Optional[List[int]] = None) -> Dict:
        ...
        try:
            ...
            recent_form_scores = []
            for player in players_data:
                recent_form = self._get_player_recent_form(player['tenniscores_player_id'], league_id)
            ...
            if opponent_player_ids and len(opponent_player_ids) == 2:
                ...
                if len(opponent_players_data) == 2:
                    h2h_score = self._calculate_head_to_head(players_data, opponent_players_data, league_id)
                else:
    ...
    def _get_player_recent_form(self, tenniscores_player_id: str, league_id: Optional[int] = None) -> float:
        ...
        try:
            ...
            """
            params = [tenniscores_player_id, tenniscores_player_id, 
            ...
            for match in recent_matches:
                is_home = (tenniscores_player_id in [match['home_player_1_id'], match['home_player_2_id']])
    ...
    def _calculate_head_to_head(self, team_a_players: List[Dict], team_b_players: List[Dict], league_id: Optional[int] = None) -> float:
        ...
        try:
            ...
            if total_matches >= 3:
                ...
                team_a_win_percentage = (recent_win_percentage * 0.6) + (team_a_win_percentage * 0.4)
            
            print(f"[DEBUG] Head-to-head: Team A won {team_a_wins}/{total_matches} matches ({team_a_win_percentage:.1f}%)")
        ...
        for metric_name, value in metrics.items():
            if metric_name in normalization_params:
                ...
                if params['max'] != params['min']:
                    normalized_value = (clamped_value - params['min']) / (params['max'] - params['min'])
                else:
                    normalized_value = 0.5
                
                normalized[metric_name] = normalized_value
            else:
                normalized[metric_name] = min(1.0, max(0.0, value / 100.0))  # Default normalization
        ...
        try:
            ...
            for metric_name, weight in self.metric_weights.items():
                if metric_name in normalized_metrics:
                    ...
                    print(f"[DEBUG] {metric_name}: {metric_value:.3f} * {weight:.3f} = {weighted_contribution:.3f}")
    ...
    def _calculate_win_probability(self, score_a: float, score_b: float) -> float:
        ...
        try:
            ...
            win_probability_pct = max(20.0, min(80.0, win_probability_pct))  # Keep between 20-80%
    ...
    def _analyze_advantages(self, team_a_metrics: Dict, team_b_metrics: Dict, 
        ...
        try:
            ...
            # Compare PTI
            pti_diff = team_a_metrics.get('average_pti', 0) - team_b_metrics.get('average_pti', 0)
            if pti_diff > 2:
                advantages['team_a_advantages'].append(f'Higher average PTI ({team_a_metrics["average_pti"]:.1f} vs {team_b_metrics["average_pti"]:.1f})')
            elif pti_diff < -2:
                advantages['team_b_advantages'].append(f'Higher average PTI ({team_b_metrics["average_pti"]:.1f} vs {team_a_metrics["average_pti"]:.1f})')
            
            # Compare win rates
            wr_diff = team_a_metrics.get('individual_win_rates', 0) - team_b_metrics.get('individual_win_rates', 0)
            if wr_diff > 10:
                advantages['team_a_advantages'].append(f'Better individual win rates ({team_a_metrics["individual_win_rates"]:.1f}% vs {team_b_metrics["individual_win_rates"]:.1f}%)')
            elif wr_diff < -10:
                advantages['team_b_advantages'].append(f'Better individual win rates ({team_b_metrics["individual_win_rates"]:.1f}% vs {team_a_metrics["individual_win_rates"]:.1f}%)')
            
            # Compare recent performance
            form_diff = team_a_metrics.get('recent_individual_form', 0) - team_b_metrics.get('recent_individual_form', 0)
            if form_diff > 15:
            ...
            # Compare experience
            exp_diff = team_a_metrics.get('experience_level', 0) - team_b_metrics.get('experience_level', 0)
            if exp_diff > 10:
    ...
    def _determine_confidence(self, team_a_metrics: Dict, team_b_metrics: Dict, win_probability: float) -> str:
        """Determine confidence based on data quality and prediction margin."""
        try:
            confidence_factors = 0
            
            # Data quality factors
            avg_experience = (team_a_metrics.get('experience_level', 0) + team_b_metrics.get('experience_level', 0)) / 2
            ...
            if team_a_metrics.get('average_pti', 0) > 15 and team_b_metrics.get('average_pti', 0) > 15:
                confidence_factors += 1
        ...
        try:
            ...
            for metric_name, weight in self.metric_weights.items():
                ...
                breakdown[metric_name] = {
                    ...
                    'weight': weight,
                    'advantage': 'Team A' if team_a_value > team_b_value else 'Team B' if team_b_value > team_a_value else 'Even'
                }
...
def get_teams_for_selection(user_league_id: Optional[str] = None, user_series: Optional[str] = None, user_club: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        if isinstance(user_league_id, str) and user_league_id != '':
            try:
                ...
                if league_record:
                    league_id_int = league_record['id']
                    print(f"[DEBUG] get_teams_for_selection: Converted league_id '{user_league_id}' to integer: {league_id_int}")
                else:
                    print(f"[WARNING] get_teams_for_selection: League '{user_league_id}' not found in leagues table")
            except Exception as e:
                print(f"[DEBUG] get_teams_for_selection: Could not convert league ID: {e}")
        elif isinstance(user_league_id, int):
            league_id_int = user_league_id
            print(f"[DEBUG] get_teams_for_selection: League_id already integer: {league_id_int}")
        ...
        print(f"[DEBUG] get_teams_for_selection: Found {len(teams)} teams for league_id {league_id_int}, series '{user_series}'")
...
def get_players_by_team(team_name: str, user_league_id: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        if isinstance(user_league_id, str) and user_league_id != '':
            try:
                ...
                if league_record:
                    league_id_int = league_record['id']
                    print(f"[DEBUG] get_players_by_team: Converted league_id '{user_league_id}' to integer: {league_id_int}")
                else:
                    print(f"[WARNING] get_players_by_team: League '{user_league_id}' not found in leagues table")
            except Exception as e:
                print(f"[DEBUG] get_players_by_team: Could not convert league ID: {e}")
        elif isinstance(user_league_id, int):
            league_id_int = user_league_id
            print(f"[DEBUG] get_players_by_team: League_id already integer: {league_id_int}")
        
        # Parse team name to extract club and series
        # Team names are typically like "Tennaqua - 22" or "Skokie Country Club - 22"
        if ' - ' not in team_name:
            print(f"[WARNING] get_players_by_team: Invalid team name format: '{team_name}'")
            return []
        
        parts = team_name.split(' - ')
        if len(parts) != 2:
            print(f"[WARNING] get_players_by_team: Could not parse team name: '{team_name}'")
            return []
        ...
        # This handles the difference between team names in series_stats and actual series names
        if series_suffix.isdigit() or series_suffix.replace('A', '').replace('B', '').isdigit():
        ...
        print(f"[DEBUG] get_players_by_team: Looking for players from club '{club_name}' in series '{series_name}' (mapped from '{series_suffix}')")
        ...
        print(f"[DEBUG] get_players_by_team: Found {len(players)} players for team '{team_name}'")
    ...
    try:
        ...
        if isinstance(user_league_id, str) and user_league_id != '':
            try:
                ...
                if league_record:
                    league_id_int = league_record['id']
                    print(f"[DEBUG] get_players_for_selection: Converted league_id '{user_league_id}' to integer: {league_id_int}")
                else:
                    print(f"[WARNING] get_players_for_selection: League '{user_league_id}' not found in leagues table")
            except Exception as e:
                print(f"[DEBUG] get_players_for_selection: Could not convert league ID: {e}")
        elif isinstance(user_league_id, int):
            league_id_int = user_league_id
            print(f"[DEBUG] get_players_for_selection: League_id already integer: {league_id_int}")
