...
def serve_mobile_matchup_simulator():
    ...
    try:
        ...
        # Get available teams for selection (filtered by user's league and series)
        available_teams = get_teams_for_selection(
            user_league_id, user_series, user_club
        )
        print(
            f"[DEBUG] serve_mobile_matchup_simulator: Found {len(available_teams)} teams in user's series '{user_series}'"
        )
...
@mobile_bp.route("/debug/team-aliases")
@login_required  
def debug_team_aliases():
    """Debug route to check team aliases in database"""
    try:
        user_league_id = session["user"].get("league_id")
        
        # Convert to league_id_int
        league_id_int = None
        if isinstance(user_league_id, str) and user_league_id != "":
            try:
                league_record = execute_query_one(
        ...
        query += " ORDER BY c.name, s.name LIMIT 20"
        
        teams = execute_query(query, params)
        
        html = "<h2>Team Aliases Debug</h2><ul>"
        for team in teams:
            html += f"<li><strong>ID:</strong> {team['id']} | <strong>Name:</strong> {team['team_name']} | <strong>Alias:</strong> {team['team_alias']} | <strong>Display:</strong> {team['display_name']} | <strong>Club:</strong> {team['club_name']} | <strong>Series:</strong> {team['series_name']}</li>"
        html += "</ul>"
